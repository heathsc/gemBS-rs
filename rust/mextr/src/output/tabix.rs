...
impl TabixFile {
    ...
    fn open_file(&mut self, line: &mut Vec<u8>) {
        if let Ok(m) = metadata(&self.name)
            && m.len() > 0
        {
            self.file = Some(HtsFile::new(&self.name, "rz").unwrap_or_else(|e| {
                panic!("tabix_thread: could not open file {}: {}", &self.name, e)
            }));
            self.process(line);
        }
    }
    ...
    fn process(&mut self, line: &mut Vec<u8>) {
        let bgzf = self
            ...
            .unwrap()
            .bgzf()
            .expect("tabix_thread: file not bgzip compressed");
        let meta_char = b'#';
        self.error = loop {
            match bgzf.getline(b'\n', line) {
                ...
                Ok(_) => {
                    ...
                    if self.idx.is_none() {
                        self.idx = Some(
                            HtsIndex::init(0, HTS_FMT_TBI, self.last_off, 14, 5)
                                .expect("tabix thread - could not init index"),
                        );
                    }
                    // Parse line
                    let seq_pos = {
                        ...
                        for (ix, s) in line.split(|c| *c == b'\t').take(2).enumerate() {
                            if ix == 0 {
                                ...
                                pos = <usize>::from_str(
                                    from_utf8(s).expect("Non UTF8 characters found"),
                                )
                                .ok()
                            }
                            // positions must be > 0
                    ...
                    if let Some((seq, pos)) = seq_pos {
                        if seq != &self.prev_ctg[..] {
                            ...
                            if self
                                .ctgs
                                .insert(self.prev_ctg.clone(), self.ctgs.len())
                                .is_some()
                            {
                                panic!("tabix_index: File {} is not sorted", &self.name)
                            }
                            debug!(
                                "Tabix thread: Added ctg {} with id {} for file {}",
                                from_utf8(&self.prev_ctg).unwrap(),
                                self.ctgs.get(&self.prev_ctg).unwrap(),
                                &self.name
                            );
                            self.last_x = 0;
                        }
                        if pos <= self.last_x {
                            panic!("tabix_index: File is not sorted")
                        }
                        self.last_x = pos;
                        if self
                            .idx
    ...
    fn finish(&mut self) {
        if !self.error {
            ...
            if idx.finish(bgzf.tell() as u64).is_err() {
                error!("Error returned from hts_idx_finish")
            } else {
                ...
                if idx.set_meta(&meta).is_err() {
                    error!("Error retruned from hts_idx_set_meta")
                } else if idx.save_as(&self.name, HTS_FMT_TBI).is_err() {
                    error!("Error retruned from hts_idx_save_as")
                }
            }
        }
    }
...
pub fn tabix_thread(chash: Arc<ConfHash>, r: Receiver<bool>) {
    ...
    for mut m in tabix_files.drain(..) {
        m.finish()
    }
