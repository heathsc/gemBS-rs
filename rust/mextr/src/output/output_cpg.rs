...
use super::{
    GT_IUPAC, GT_MASK, MethRec, OutputOpts, Record, calc_phred, get_prob_dist, output_mrec,
};
use crate::config::*;
    ...
    if mrec1.max_gt().is_some() && mrec2.max_gt().is_some() {
        ...
        if ct1[5] + ct1[7] >= opts.min_inform() || ct2[6] + ct2[4] >= opts.min_inform() {
            if matches!(opts.select, Select::Hom) {
                ...
                (
                    HET_C[gt1 as usize] && HET_G[gt2 as usize],
                    (p1[1] + p1[4] + p1[5] + p1[6] + p2[2] + p2[5] + p2[7] + p2[8]).exp(),
                )
            }
        } else {
            (false, 0.0)
...
fn output_combined_mrec<W: Write>(f: &mut W, mrec1: &MethRec, mrec2: &MethRec) -> io::Result<()> {
    if let (Some(gt1), Some(gt2)) = (mrec1.max_gt(), mrec2.max_gt()) {
        let gq = calc_phred(
            1.0 - (mrec1.gt_probs()[gt1 as usize] + mrec2.gt_probs()[gt2 as usize]).exp(),
        );
        ...
        write!(
            f,
            "\t{}{}\tGQ={};MQ={}",
            GT_IUPAC[gt1 as usize] as char, GT_IUPAC[gt2 as usize] as char, gq, mq
        )?; // TODO - fix mq calc
        if gt1 != 4 || gt2 != 7 {
            write!(
                f,
                ";DQ={}",
                calc_phred((mrec1.gt_probs()[4] + mrec2.gt_probs[7]).exp())
            )?
        }

        // Collect counts
        let (ct0, ct1) = (
            mrec1.counts[5] + mrec2.counts[6],
            mrec1.counts[7] + mrec2.counts[4],
        );
        let (ct2, ct3) = {
            let mut x = 0;
            let mut m = 1;
        ...
        let (m1, m2) = (
            mrec1.get_meth(Strand::C).unwrap_or(-1.0),
            mrec2.get_meth(Strand::G).unwrap_or(-1.0),
        );
        let (wt1, wt2) = (
            if m1 < 0.0 { 0.0 } else { fn1 },
            if m2 < 0.0 { 0.0 } else { fn2 },
        );
        // Quick approx - simply get the ratio of psuedocounts
        if wt1 + wt2 > 0.0 {
            write!(f, "\t{:.3}", (m1 * wt1 + m2 * wt2) / (wt1 + wt2))?
        } else {
            write!(f, "\t.")?
        };
...
fn output_separate_single_rec<W: Write>(
    f: &mut W,
    hdr: &VcfHeader,
    rec: &Record,
    meth_rec: &MethRec,
    ...
    write!(
        f,
        "{}\t{}\t{}\t{}",
        hdr.ctg_name(rec.rid as usize).unwrap(),
        rec.pos,
        rec.pos + 1,
        rec.cx[2] as char
    )?;
...
fn output_separate_multi_rec<W: Write>(
    f: &mut W,
    hdr: &VcfHeader,
    rec: &Record,
    mvec: &[MethRec],
    ...
    for meth_rec in mvec.iter() {
        output_mrec(f, meth_rec, strand, false)?
    }
    writeln!(f)
}

fn output_combined_single_rec<W: Write>(
    f: &mut W,
    hdr: &VcfHeader,
    rec1: &Record,
    rec2: &Record,
    ...
    write!(
        f,
        "{}\t{}\t{}\t{}{}",
        hdr.ctg_name(rec1.rid as usize).unwrap(),
        rec1.pos,
        rec2.pos + 1,
        rec1.cx[2] as char,
        rec1.cx[3] as char
...
fn output_combined_multi_rec<W: Write>(
    f: &mut W,
    hdr: &VcfHeader,
    rec1: &Record,
    rec2: &Record,
    ...
    for (meth_rec1, meth_rec2) in mvec1.iter().zip(mvec2.iter()) {
        output_combined_mrec(f, meth_rec1, meth_rec2)?
    }
    writeln!(f)
}

fn output_single_rec<'a, W: Write>(
    f: &mut W,
    hdr: &VcfHeader,
    opts: &OutputOpts,
    srec: &'a [(Record, MethRec)],
    mut prev: Option<(&'a Record, &'a MethRec)>,
) -> io::Result<()> {
            ...
            && rec1.pos + 1 == rec2.pos
        {
            ...
            if pass {
                ...
                if opts.min_n() < 1 || phred >= opts.threshold {
                    if matches!(opts.mode, Mode::Combined) {
                        output_combined_single_rec(f, hdr, rec1, rec2, meth_rec1, meth_rec2)?
                    } else {
...
fn output_multi_rec<'a, W: Write>(
    f: &mut W,
    hdr: &VcfHeader,
    opts: &OutputOpts,
    srec: &'a [(Record, Box<[MethRec]>)],
    mut prev: Option<(&'a Record, &'a [MethRec])>,
) -> io::Result<()> {
    let ns = hdr.nsamples();
    ...
    for (rec2, mvec2) in srec {
            ...
            && rec1.pos + 1 == rec2.pos
        {
            ...
            mvec1
                .iter()
                .zip(mvec2.iter())
                .map(|(m1, m2)| calc_prob(m1, m2, opts))
                .for_each(|(_, q)| qvec.push(q));
            ...
            if opts.min_n == 0 || phred >= opts.threshold() {
                if matches!(opts.mode, Mode::Combined) {
                    output_combined_multi_rec(f, hdr, rec1, rec2, mvec1, mvec2)?
                } else {
...
pub fn output_cpg(
    outfiles: &mut [HtsFile],
    rec_blk: &RecordBlock,
    prev: Option<RecordBlockElem>,
    chash: &ConfHash,
    hdr: &VcfHeader,
) -> io::Result<()> {
    ...
    match rec_blk {
        RecordBlock::Single(svec) => {
            let prev = if let Some(RecordBlockElem::Single((r, m))) = prev {
                Some((r, m))
            } else {
                None
            };
        ...
        RecordBlock::Multi(mvec) => {
            let prev = if let Some(RecordBlockElem::Multi((r, mv))) = prev {
                Some((r, mv as &[MethRec]))
            } else {
                None
            };
