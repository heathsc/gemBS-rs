...
impl BbiWriter {
    pub fn fp(&mut self) -> &mut BufWriter<File> {
        &mut self.fp
    }
    pub fn header(&mut self) -> &mut BbiHeader {
        &mut self.header
    }
    pub fn init(bbi_file: &BbiFile, bbi_type: BbiBlockType, n_ctgs: usize) -> Self {
        ...
        Self {
            ...
            name,
            bbi_type,
            zoom_data: Default::default(),
            fp,
            index_offset: 0,
        }
    }
    pub fn bbi_type(&self) -> BbiBlockType {
        self.bbi_type
    }
    pub fn ctg_blocks(&self) -> &[Vec<BbiCtgBlock>] {
        &self.ctg_blocks
    }
...
fn init_writers(ch: &ConfHash) -> (Vec<BbiWriter>, Vec<BbiWriter>) {
    ...
    let mut bb_writers: Vec<_> = bbi
        .bb_files()
        .iter()
        .enumerate()
        .map(|(i, f)| BbiWriter::init(f, BbiBlockType::Bb(i as u8), n_ctgs))
        .collect();
    trace!("init bw writers");
    let mut bw_writers: Vec<_> = bbi
        .bw_files()
        .iter()
        .enumerate()
        .map(|(i, f)| BbiWriter::init(f, BbiBlockType::Bw(i as u8), n_ctgs))
        .collect();
...
fn add_block(ws: &mut WriterState, blk: &BbiBlock) {
    let pos = ws
        ...
        .fp
        .stream_position()
        .expect("Error getting position from BBI file");
...
pub fn write_bbi_thread(ch: Arc<ConfHash>, r: Receiver<BbiMsg>) {
    ...
    for (ix, w) in bb_writers.iter_mut().enumerate() {
        state.insert(BbiBlockType::Bb(ix as u8), WriterState::new(w));
    }
    for (ix, w) in bw_writers.iter_mut().enumerate() {
        state.insert(BbiBlockType::Bw(ix as u8), WriterState::new(w));
    }
    ...
    for msg in r.iter() {
        match msg {
            BbiMsg::Data((blk, v)) => {
                ...
                if blk.idx() == ws.curr_idx {
                    ...
                    ws.writer
                        .fp
                        .write_all(&v)
                        .expect("Couldn't write out compressed block");
                    drop(v);
                    ws.curr_idx += 1;
            ...
            BbiMsg::EndOfSection((bbi_type, ix, v)) => {
                ...
                debug!(
                    "writer for bbi_type: {:?} got end of section with index {} {:?} (curr_index: {}, {:?})",
                    bbi_type, ix, v, ws.curr_idx, ws.curr_zoom_idx
                );
                if ws.check_idx(ix, &v) {
                    ...
                    debug!(
                        "writer for bbi_type: {:?} waiting for index {} (curr_index: {}, {:?})",
                        bbi_type, ix, ws.curr_idx, ws.curr_zoom_idx
                    );
                }
            }
        }
    ...
    for (bbi_type, ws) in state.iter() {
        debug!(
            "Writer state: {:?} {} {:?}",
            bbi_type, ws.curr_idx, ws.curr_zoom_idx
        );
