...
fn read_header(infile: &str) -> io::Result<(usize, Vec<VcfContig>)> {
    ...
    let ns = hdr.nsamples();
    if ns == 0 {
        Err(new_err(format!("No samples in input file {}", infile)))
    } else {
        let nctgs = hdr.nctgs();
        if nctgs == 0 {
            Err(new_err(format!("No contigs in input file {}", infile)))
        } else {
            let mut v = Vec::new();
            for ix in 0..nctgs {
...
fn setup_bed_methyl_structs(
    ...
    if bed_meth {
        if hdr.nsamples() > 1 {
            return Err(new_err(format!("Input file {} has {} samples: bedMethyl output incompatible with multi-sample files", infile, hdr.nsamples())));
        }
        // Get sample description from VCF header if possible, otherwise use sample name
        let quotes = ['\'', '\"'];
        let trim = |s: &str| {
            s.trim_start_matches(&quotes[..])
        ...
        chash.set(
            "sample_desc",
            ConfVar::String(sample_desc.or_else(|| Some(sample_name.as_ref().unwrap().to_owned()))),
        );
        chash.set("sample_name", ConfVar::String(sample_name));
    } else {
        chash.set("sample_desc", ConfVar::String(None))
    }
    Ok(())
}
...
pub fn handle_options(m: &ArgMatches) -> io::Result<(ConfHash, BcfSrs)> {
    ...
    // Check threads
    if chash.get_int("threads") < 1 {
        chash.set("threads", ConfVar::Int(1))
    }

    // Check threshold
    if chash.get_int("threshold") > 255 {
        chash.set("threshold", ConfVar::Int(255))
    }
