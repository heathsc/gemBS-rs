...
impl SnpBlock {
    pub fn new(contig: Arc<Contig>, snps: Vec<RawSnp>) -> Self {
        Self { contig, snps }
    }
    pub fn contig(&self) -> Arc<Contig> {
        self.contig.clone()
    }
    pub fn snps(&self) -> &[RawSnp] {
        &self.snps
    }
    ...
    pub fn min_max(&self) -> Option<(u32, u32)> {
        ...
        it.next().map(|x| {
            it.fold((x.pos(), x.pos()), |(a, b), y| {
                (a.min(y.pos()), b.max(y.pos()))
            })
        })
    }
}
...
impl<'a> SnpBuilder<'a> {
    ...
    pub fn mk_snp(&mut self, name: &str, ctg: &str, pos: u32, maf: Option<f32>) -> Option<Snp> {
        if let Some(tname) = name.strip_prefix("rs") {
            if tname.is_empty() || tname.find(|c: char| !char::is_ascii_digit(&c)).is_some() {
                None
            } else {
                self.build_snp(tname, ctg, pos, maf)
            }
        } else {
            None
        }
...
fn store_snp_block(sb: &SnpBlock, data: &mut ContigData, conf: &Config) {
    for snp in sb.snps().iter() {
        data.add_snp(snp, conf);
    }
}
            ...
            if let Ok(op) = sel.ready_timeout(Duration::from_millis(100)) {
                match op {
                    idx if idx == ctr_idx => match control_receiver.recv() {
                        ...
                        Err(e) => panic!(
                            "Store thread {} - Error receiving message from control channel: {}",
                            thread_id, e
                        ),
                    },
                    idx => {
