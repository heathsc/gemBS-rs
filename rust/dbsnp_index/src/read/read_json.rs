...
fn handle_json_tokens<'a>(
    jtxt: &'a str,
    jtok: &[JTok],
    jsnp: &mut JsonSnp<'a>,
    level: usize,
) -> usize {
    if jtok.is_empty() || (jsnp.mask & VALID_SNP) != 0 {
        0
    } else {
        match jtok[0].tok_type {
            ...
            JType::Object => {
                ...
                for _ in 0..jtok[0].size {
                    ...
                    if ntok.size > 0
                        && let Some(key) = JS_KEYS.get(&jtxt[ntok.start..ntok.end])
                    {
                        let ntok1 = &jtok[j + 2];
                        match key {
                            JsKey::RefsnpId => {
                            ...
                            JsKey::PrimarySnapshotData => {
                                if level == 0 && ntok1.tok_type == JType::Object {
                                    ...
                                    if let (Some(s1), Some(s2)) =
                                        (jsnp.inserted_sequence, jsnp.deleted_sequence)
                                        && s1 != s2
                                        && jsnp.pos.is_some()
                                    {
                                        jsnp.mask |= VALID_SNP;
                                        if jsnp.total_count > 0 {
                                            let z = (jsnp.allele_count as f32)
                                                / (jsnp.total_count as f32);
                            ...
                            JsKey::PlacementsWithAllele => {
                                if level == 1
                                    && ntok1.tok_type == JType::Array
                                    && (jsnp.mask & IN_PRIMARY_SNAPSHOT_DATA) != 0
                                {
                            ...
                            JsKey::IsPtlp => {
                                if level == 3
                                    && (jsnp.mask & IN_PLACEMENTS_WITH_ALLELE) != 0
                                    && let JType::Bool(x) = ntok1.tok_type
                                {
                                    if x {
                                        jsnp.mask |= IS_PTLP
                                    } else {
                            ...
                            JsKey::Frequency => {
                                if level == 3
                                    && ntok1.tok_type == JType::Array
                                    && (jsnp.mask & IN_ALLELE_ANNOTATIONS) != 0
                                {
                                    jsnp.mask = (jsnp.mask & !FREQ_FLAGS) | IN_FREQUENCY;
                                    j += 2;
                                    for _ in 0..ntok1.size {
                                        j += handle_json_tokens(
                                            jtxt,
                                            &jtok[j + 1..],
                                            jsnp,
                                            level + 2,
                                        );
                                        if (jsnp.mask & FREQ_FLAGS) == FREQ_FLAGS
                                            && jsnp.a <= jsnp.b
                                        {
                            ...
                            JsKey::Alleles => {
                                if level == 3
                                    && ntok1.tok_type == JType::Array
                                    && (jsnp.mask & (IN_PLACEMENTS_WITH_ALLELE | IS_PTLP))
                                        == (IN_PLACEMENTS_WITH_ALLELE | IS_PTLP)
                                {
                            ...
                            JsKey::Allele => {
                                if level == 5
                                    && ntok1.tok_type == JType::Object
                                    && (jsnp.mask & IN_ALLELES) != 0
                                {
                                    jsnp.mask |= IN_ALLELE;
                            ...
                            JsKey::Observation => {
                                if level == 5
                                    && ntok1.tok_type == JType::Object
                                    && (jsnp.mask & IN_FREQUENCY) != 0
                                {
                                    jsnp.mask = (jsnp.mask & !FREQ_ALLELE_FLAGS)
                                        | (IN_OBSERVATION | FREQ_ALLELES_OK);
                                    j += 1 + handle_json_tokens(
                                        jtxt,
                                        &jtok[j + 2..],
                                        jsnp,
                                    ...
                                    if (jsnp.mask & (FREQ_ALLELE_FLAGS | FREQ_ALLELES_OK))
                                        != (FREQ_ALLELE_FLAGS | FREQ_ALLELES_OK)
                                    {
                                        jsnp.mask &=
                                            !(FREQ_ALLELE_FLAGS | FREQ_ALLELES_OK | IN_OBSERVATION)
                                    } else {
                            ...
                            JsKey::InsertedSequence => {
                                if level == 7
                                    ...
                                    && (jsnp.mask & IN_SPDI) != 0
                                {
                                    if ntok1.end - ntok1.start == 1 {
                                        jsnp.inserted_sequence =
                                            Some(jtxt.as_bytes()[ntok1.start..ntok1.end][0])
                                    }
                                } else if level == 6
                                    && ntok1.tok_type == JType::String
                                    && (jsnp.mask & IN_OBSERVATION) != 0
                                {
