...
impl ContigBin {
    pub fn name_buf(&self) -> &[u8] {
        &self.name_buf
    }
    pub fn mask(&self) -> &[u128; 2] {
        &self.mask
    }
    ...
    pub fn write_names<W: Write>(&mut self, w: W) {
        ...
        for (_, x) in idx.iter() {
            ...
            if (x & 1) == 1 {
                writer.write_u4(*it.next().expect("Short name buffer"))
            }
            loop {
                match it.next() {
                    None => break,
...
impl<W: Write> NameWriter<W> {
    ...
    fn write_u8(&mut self, c: u8) {
        if let Some(x) = self.buf {
            ...
            self.writer.write_all(&[c]).expect("Write error")
        }
    }
}
...
impl ContigStats {
    ...
    pub fn n_selected_snps(&self) -> usize {
        self.n_selected_snps
    }
    pub fn n_non_empty_bins(&self) -> usize {
        self.n_non_empty_bins
    }
}
...
impl ContigData {
    pub fn recv(&self) -> &Receiver<SnpBlock> {
        &self.recv
    }
    pub fn check_bins(&mut self, min: u32, max: u32) {
        ...
        if let Some(idata) = self.inner.as_mut() {
            ...
            self.inner = Some(ContigInnerData::new(min_bin, max_bin))
        }
    }
    pub fn add_snp(&mut self, snp: &RawSnp, conf: &Config) {
        ...
        if (bin.mask[ix] & mask) == 0 {
            ...
            let name = snp.name();
            if name.len() > 254 {
                panic!("The name for SNP {} is too long (Max <= 254)", name)
            }
            let select = {
                let s = if let Some(maf_limit) = conf.maf_limit() {
                    if let Some(maf) = snp.maf() {
                        maf >= maf_limit as f32
                    } else {
                        false
                    }
                } else {
            ...
            for v in it {
                bin.name_buf.push(((v[0] - b'0') << 4) | (v[1] - b'0'))
            }
            if rem.is_empty() {
                ...
                bin.name_buf
                    .push(((rem.first().unwrap() - b'0') << 4) | term_code);
                bin.half_full = false;
            }
    ...
    pub fn min_bin(&self) -> Option<usize> {
        self.inner.as_ref().map(|x| x.min_bin)
    }
    pub fn max_bin(&self) -> Option<usize> {
        self.inner.as_ref().map(|x| x.max_bin)
    }
    pub fn min_max(&self) -> Option<(usize, usize)> {
        self.inner.as_ref().map(|x| (x.min_bin, x.max_bin))
    }
    pub fn bins(&mut self) -> Option<&mut Vec<Option<ContigBin>>> {
        if let Some(idata) = &mut self.inner {
            Some(&mut idata.bins)
        } else {
            None
        }
...
impl Contig {
    pub fn new<S: AsRef<str>>(name: S, channel_size: usize) -> Self {
        ...
        let data = RwLock::new(ContigData {
            recv,
            inner: None,
    ...
    pub fn send_message(&self, sb: SnpBlock) {
        self.send.send(sb).unwrap();
    }
    ...
    pub fn data(&self) -> &RwLock<ContigData> {
        &self.data
    }
}
...
impl ContigHash {
    pub fn new(channel_size: usize, chrom_alias: Option<HashMap<String, String>>) -> Self {
        Self {
            contig_hash: RwLock::new(HashMap::new()),
            chrom_alias,
            channel_size,
        }
    }
