...
impl KnownVarList {
    ...
    pub fn check_vtype(&self, name: &str, section: Section) -> Option<VarType> {
        ...
        self.known_var.get(&tstr.as_str()).and_then(|v| {
            if v.sections.contains(&section) {
                Some(v.vtype)
            } else {
                None
            }
...
fn make_known_var_list() -> KnownVarList {
    ...
    kv_list.add_known_var(
        "merge_threads",
        VarType::Int,
        vec![Section::Mapping, Section::Calling],
    );
    kv_list.add_known_var(
        "merge_cores",
        VarType::Int,
    ...
    kv_list.add_known_var(
        "mapq_threshold",
        VarType::Int,
        vec![Section::Calling, Section::Report],
    );
    ...
    kv_list.add_known_var(
        "max_template_length",
        VarType::IntVec,
        vec![Section::Calling, Section::Mapping],
    );
    ...
    kv_list.add_known_var(
        "reference_bias",
        VarType::Float,
        vec![Section::Calling, Section::Extract],
    );
    kv_list.add_known_var(
...
impl PrepConfig {
    ...
    fn handle_value(&mut self, tok: LexToken, name: String) -> Result<ParserState, String> {
        match tok {
            LexToken::Value(val_str) => {
                let section = if let Some(s) = self.lexer.get_section() {
                    s
                } else {
                    return Err("Internal error - no section".to_string());
                };
                ...
                let rv = match vt {
                    VarType::FloatVec | VarType::StringVec | VarType::IntVec => {
                        DataValue::StringVec(vec![val_str; 1])
                    }
                    _ => DataValue::String(val_str),
                };

                let pvar = PrepConfigVar {
                    var: rv,
                    vtype: vt,
                    section,
                    known,
            ...
            _ => Err(format!(
                "Unexpected token - waiting for value after variable {}",
                name
            )),
        }
    }

    fn handle_next_value(
        &mut self,
        val_str: String,
        mut pvar: PrepConfigVar,
        name: String,
        section: Section,
    ...
    fn handle_after_value(
        &mut self,
        tok: LexToken,
        x: (String, Section, PrepConfigVar),
    ) -> Result<ParserState, String> {
        let name = x.0;
        let section = x.1;
        let pvar = x.2;
        ...
        match tok {
            ...
            _ => Err(format!(
                "Unexpected token - waiting for variable name or value after variable {}",
                name
            )),
        }
    }
            ...
            match seg {
                Segment::NameOnly(idx) => {
                    ...
                    match stored_var {
                        ...
                        None => warn!(
                            "Unknown variable '{}' in assignment for variable '{}'",
                            vname, name
                        ),
                    }
                }
                Segment::SectionName(idx) => {
                    ...
                    let sec = if idx[3] != idx[2] {
                        match Section::from_str(var_str.get(idx[2]..idx[3]).unwrap()) {
                            ...
                            Err(_) => {
                                return Err(format!(
                                    "Unknown Section '{}' in assignment for variable '{}'",
                                    vname, name
                                ))
                            }
                        }
        ...
        for (name, v) in self.var.iter_mut() {
            for pv in v {
                if pv.known {
                    let rv = match pv.vtype {
                        ...
                        VarType::FloatVec => {
                            if let DataValue::StringVec(vv) = &pv.var {
                                let mut v = Vec::new();
                                for s in vv.iter() {
                                    if let DataValue::Float(val) =
                                        DataValue::from_str(s, VarType::Float)?
                                    {
                                        v.push(val);
                                    }
                                }
                        ...
                        VarType::IntVec => {
                            if let DataValue::StringVec(vv) = &pv.var {
                                let mut v = Vec::new();
                                for s in vv.iter() {
                                    if let DataValue::Int(val) =
                                        DataValue::from_str(s, VarType::Int)?
                                    {
                                        v.push(val);
                                    }
                                }
                    ...
                    warn!(
                        "Warning: Variable '{}' in section '{:?}' not used",
                        name, pv.section
                    );
                }
            }
