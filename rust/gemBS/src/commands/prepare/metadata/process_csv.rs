...
use crate::common::defs::{DataValue, Metadata, VarType};
            ...
            match Metadata::from_str(field) {
                Ok(md) => {
                    if let Some(x) = header.columns.get(&md) {
                        return Err(format!(
                            "Column {} clashes with column {} in cvs metadata file",
                            x + 1,
                            ix + 1
                        ));
                    } else {
                        header.columns.insert(md, ix);
                    }
                }
                Err(_) => {
                    warn!(
                        "Warning: column {} in cvs metadata file not recognized",
                        field
                    );
                }
            }
        }
        if header.columns.contains_key(&Metadata::SampleBarcode)
            && header.columns.contains_key(&Metadata::Dataset)
        {
            Ok(header)
        } else {
            Err(
                "Error: CVS metadata file does not contain sample barcode and dataset columns"
                    .to_string(),
            )
        }
    }
...
impl InputLine {
    ...
    fn insert(&mut self, key: Metadata, val: DataValue) -> Result<(), &'static str> {
        ...
        if (self.mask & 3) == 3 {
            return Err("can not have FilePath in the same record as FilePath1 or FilePath2");
        }
        if (self.mask & 6) == 6 {
            return Err("can not have ReadEnd in the same record as FilePath1 or FilePath2");
        }
        self.vals.insert(key, val);
        Ok(())
    }
    fn get_hash(&mut self) -> &mut HashMap<Metadata, DataValue> {
        &mut self.vals
    }
}
...
pub fn process_cvs_metatdata_file(file_name: &str, gem_bs: &mut GemBS) -> Result<(), String> {
    let mut rdr = Reader::from_path(file_name).map_err(|e| {
        format!(
            "Error opening cvs metadata file {} for input: {}",
            file_name, e
        )
    })?;
    let hrecord = rdr.headers().map_err(|e| {
        format!(
            "Error reading header line from cvs metadata file {} for input: {}",
            file_name, e
        )
    })?;
        ...
        match rdr.read_record(&mut record) {
            Ok(tst) => {
                        ...
                        match DataValue::from_str(field, *MD_ASSOC.get(md).unwrap()) {
                            Ok(val) => {
                                if let Err(e) = input_line.insert(*md, val) {
                                    return Err(format!(
                                        "Error reading from cvs metadata file {}, line {}: {}",
                                        file_name, line, e
                                    ));
                                }
                            }
                            Err(_) => {
                                return Err(format!(
                                    "Error reading from cvs metadata file {}, line {}, col {}",
                                    file_name,
                                    line,
                                    ix + 1
                                ));
                ...
                if let Err(e) = sample_data.add_csv_record(input_line.get_hash()) {
                    return Err(format!(
                        "Error reading record at line {} of cvs metadata file {}: {}",
                        line, file_name, e
                    ));
                }
            }
            Err(e) => {
                return Err(format!(
                    "Error reading record at line {} of cvs metadata file {} for input: {}",
                    line, file_name, e
                ));
            }
        }
