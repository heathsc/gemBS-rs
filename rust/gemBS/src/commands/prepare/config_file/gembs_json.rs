...
fn check_and_assign_var(
    ...
    if gem_bs.get_config_strict(section, k).is_none() {
        ...
        if INDEX_PATH_VARS.contains(k.to_lowercase().as_str())
            && let Some(idir) = gem_bs.get_config_str(Section::Mapping, "index_dir")
        {
        ...
        let rv = match vt {
            ...
            VarType::FloatVec => {
                let mut v = Vec::new();
                if let DataValue::Float(val) = DataValue::from_str(&val, VarType::Float)? {
                    v.push(val);
                }
            ...
            VarType::IntVec => {
                let mut v = Vec::new();
                if let DataValue::Int(val) = DataValue::from_str(&val, VarType::Int)? {
                    v.push(val);
                }
        ...
        if let Some(var) = rv {
            gem_bs.set_config(section, k, var);
        }
    } else {
        warn!(
            "Ignored setting [{:?}]{} from gemBS JSON file {}",
            section,
            k,
            p.display()
        )
...
fn process_gembs_json_file(
    gem_bs: &mut GemBS,
    kv_list: &KnownVarList,
    path: &Path,
) -> Result<(), String> {
    let rdr = compress::open_bufreader(path)
        .map_err(|e| format!("Could not open gemBS JSON file {}: {}", path.display(), e))?;
    let json_data: GemBSJson = serde_json::from_reader(rdr).map_err(|e| {
        format!(
            "Could not parse JSON metadata file {}: {}",
            path.display(),
            e
        )
    })?;
    for (section, data) in json_data.0.iter() {
        for (k, dat) in data.iter() {
            if let Some(vt) = kv_list.check_vtype(k, *section) {
                ...
                warn!(
                    "Unknown key {} for section {:?} in gemBS JSON file {}",
                    k,
                    section,
                    path.display()
                );
...
pub fn check_gembs_json(gem_bs: &mut GemBS, kv_list: &KnownVarList) -> Result<(), String> {
    if let Some(DataValue::String(idx_dir)) = gem_bs.get_config(Section::Mapping, "index_dir") {
        let p: PathBuf = [Path::new(idx_dir), Path::new("gemBS_index.json")]
            .iter()
            .collect();
        if p.exists() {
            process_gembs_json_file(gem_bs, kv_list, &p)?;
        }
    }
    Ok(())
}
