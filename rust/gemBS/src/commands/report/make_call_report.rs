...
use crate::common::json_call_stats::{
    CallJson, Counts, FSBaseLevelType, FSCounts, FSReadLevelType, MutCounts, QCCounts,
};
...
fn make_hist(
    ...
    let mut chart = ChartBuilder::on(&root)
        ...
        .build_cartesian_2d(0.0..mx, 0.0..max)?;
    ...
    chart.draw_series(t.iter().map(|(x, y)| {
        Rectangle::new(
...
fn make_coverage_graph(
    ...
    let (name, title, xaxis, yaxis, ch) = match cov {
        ...
        CovType::RefCpgInf => (
            "ref_cpg_inf",
            "Informative Coverage at Reference Cpgs",
            "Informative Coverage",
            "# CpGs",
            &coverage.ref_cpg_inf,
        ),
        CovType::NonRefCpg => (
            "non_ref_cpg",
            "Coverage at Non Reference CpGs",
            "Coverage",
            "# CpGs",
            &coverage.non_ref_cpg,
        ),
        CovType::NonRefCpgInf => (
            "non_ref_cpg_inf",
            "Informative Coverage at Non Reference Cpgs",
            "Informative Coverage",
            "# CpGs",
            &coverage.non_ref_cpg_inf,
        ),
        CovType::Variant => (
            "variants",
    ...
    let path: PathBuf = [
        dir,
        Path::new(format!("{}_coverage_{}.png", bc, name).as_str()),
    ]
    .iter()
    .collect();
...
fn make_quality_graph(
    ...
    let (name, title, yaxis, qv) = match qual {
        ...
        QualType::RefCpg => (
            "ref_cpg",
            "Quality at Reference CpGs",
            "# CpGs",
            &quality.ref_cpg,
        ),
        QualType::NonRefCpg => (
            "non_ref_cpg",
            "Quality at Non Reference CpGs",
            "# CpGs",
            &quality.non_ref_cpg,
        ),
        QualType::Variant => (
            "variants",
            "Quality at Variant Sites",
            "# sites",
            &quality.variant,
        ),
    };
    let path: PathBuf = [
        dir,
        Path::new(format!("{}_quality_{}.png", bc, name).as_str()),
    ]
    .iter()
    .collect();
    let mut th = HashMap::new();
    for (x, y) in qv.iter().enumerate() {
        th.insert(x, *y);
    }
    if th.is_empty() {
        th.insert(0, 0);
...
fn make_qc_dist_graph(
    ...
    let (name, title, xaxis, variant, qv) = match qual {
        QCDistType::QD => (
            "qd_variant",
            "Quality by depth for variant allele",
            "Quality by Depth",
            true,
            &qc_dist.quality_by_depth,
        ),
        QCDistType::QDNonVar => (
            "qd_nonvariant",
            "Quality by depth for non-variant allele",
            "Quality by Depth",
            false,
            &qc_dist.quality_by_depth,
        ),
        QCDistType::RMS => (
            "rmsmq_variant",
            "RMS MapQ of variant allele reads",
            "RMS MapQ",
            true,
            &qc_dist.rms_mapping_quality,
        ),
        QCDistType::RMSNonVar => (
            "rmsmq_nonvariant",
            "RMS MapQ of non-variant allele reads",
            "RMS MapQ",
            false,
            &qc_dist.rms_mapping_quality,
        ),
    };
    let path: PathBuf = [dir, Path::new(format!("{}_{}.png", bc, name).as_str())]
        .iter()
        .collect();
    let mut th = HashMap::new();
    if variant {
        for (x, y) in qv.iter() {
            th.insert(*x, y.variant());
        }
    } else {
        for (x, y) in qv.iter() {
            th.insert(*x, y.non_variant());
        }
    }
    if th.is_empty() {
        th.insert(0, 0);
...
fn load_call_json(cjson: LoadCallJson) -> Result<(), String> {
    ...
    let mut cj = cjson
        .call_json
        .write()
        .expect("Couldn't obtain write lock on CallJson structure");
    *cj = Some(jstats);
...
fn make_heatmap(
    ...
    let mut chart = ChartBuilder::on(&area1)
        ...
        .margin(5)
        .caption(title, ("sans-serif", 22.0).into_font())
        .build_cartesian_2d(-0.5..100.5, 0.5..ylim as f64 + 0.5)?;
    ...
    for (y, v) in ch {
        ...
        chart.draw_series(v.iter().enumerate().map(|(x, z)| {
            let x1 = x as f64 - 0.5;
            let y1 = *y as f64 - 0.5;
    ...
    let mut chart = ChartBuilder::on(&area2)
        ...
        .margin_left(30)
        .margin_top(35)
        .build_cartesian_2d(0..1, 0..max_z)?;
...
fn make_gc_coverage_heatmap(bc: &str, dir: &Path, call_json: &CallJson) -> Result<(), String> {
    let path: PathBuf = [dir, Path::new(format!("{}_gc_coverage.png", bc).as_str())]
        .iter()
        .collect();
...
fn make_meth_level_plot(
    ...
    let mut chart = ChartBuilder::on(&root)
        ...
        .margin(5)
        .caption(title, ("sans-serif", 22.0).into_font())
        .build_cartesian_2d(0.0..100.0, 0.0..ymax)?;
    ...
    for (ix, v) in vrf.iter().enumerate() {
        ...
        chart
            .draw_series(LineSeries::new(
                v.iter().enumerate().map(|(x, y)| (x as f64, *y / tot)),
...
fn make_meth_level_chart(bc: &str, dir: &Path, call_json: &CallJson) -> Result<(), String> {
    let path: PathBuf = [
        dir,
        Path::new(format!("{}_methylation_levels.png", bc).as_str()),
    ]
    .iter()
    .collect();
...
fn make_noncpg_profile_plot(
    ...
    let mut chart = ChartBuilder::on(&root)
        ...
        .margin(5)
        .caption(title, ("sans-serif", 22.0).into_font())
        .build_cartesian_2d(0..v.len(), 0.0..max)?;
...
fn make_noncpg_read_profile(bc: &str, dir: &Path, call_json: &CallJson) -> Result<(), String> {
    let path: PathBuf = [
        dir,
        Path::new(format!("{}_non_cpg_read_profile.png", bc).as_str()),
    ]
    .iter()
    .collect();
    ...
    match rf {
        ...
        Some(v) => {
            ...
            make_noncpg_profile_plot(
                &path,
                &tv,
                "% Non-Conversion at Non-CpG Sites",
                "Position in Read",
                "% Non-Conversion",
            )
            .map_err(|e| format!("{}", e))
...
fn make_read_level_tab<T: Table>(table: &mut T, json: &CallJson, ms: Option<&mut MapSummary>) {
    ...
    for (key, name) in FSReadLevelType::iter() {
        if let Some(t) = rl.get(&key) {
            table.add_row(f(name, *t));
        }
    }
    if let Some(msumm) = ms {
        msumm.aligned = tot.bases();
        msumm.unique = tot.bases()
            - rl.get(&FSReadLevelType::LowMAPQ)
                .map(|x| x.bases())
                .unwrap_or(0);
        msumm.passed = rl
            .get(&FSReadLevelType::Passed)
            .map(|x| x.bases())
            .unwrap_or(0);
...
fn make_read_level_table(json: &CallJson, msumm: &mut MapSummary) -> Content {
    let mut table = HtmlTable::new("hor-zebra");
    make_read_level_tab(&mut table, json, Some(msumm));
    Content::Table(table)
}

fn make_read_level_latex_tab(json: &CallJson) -> LatexContent {
    let mut table = LatexTable::new();
    make_read_level_tab(&mut table, json, None);
    LatexContent::Table(table)
}

fn make_base_level_tab<T: Table>(table: &mut T, json: &CallJson) {
...
fn make_base_level_table(json: &CallJson) -> Content {
    let mut table = HtmlTable::new("green");
    make_base_level_tab(&mut table, json);
    Content::Table(table)
}

fn make_base_level_latex_tab(json: &CallJson) -> LatexContent {
    let mut table = LatexTable::new();
    make_base_level_tab(&mut table, json);
    LatexContent::Table(table)
}

fn make_variant_count_tab<T: Table>(table: &mut T, json: &CallJson) {
...
fn make_variant_count_table(json: &CallJson) -> Content {
    let mut table = HtmlTable::new("hor-zebra");
    make_variant_count_tab(&mut table, json);
    Content::Table(table)
}

fn make_variant_count_latex_tab(json: &CallJson) -> LatexContent {
    let mut table = LatexTable::new();
    make_variant_count_tab(&mut table, json);
    LatexContent::Table(table)
}

fn make_cpg_count_tab<T: Table>(table: &mut T, json: &CallJson, ms: Option<&mut MethSummary>) {
    table.add_header(vec!["Type", "Total", "Passed", "% Passed"]);
    let bs = json.basic_stats();
    let f = |s: &str, ct: &Counts, tab: &mut T, opt: bool| {
        if !opt || ct.all() > 0 {
...
fn make_cpg_count_table(json: &CallJson, msumm: &mut MethSummary) -> Content {
    let mut table = HtmlTable::new("hor-zebra");
    make_cpg_count_tab(&mut table, json, Some(msumm));
    Content::Table(table)
}

fn make_cpg_count_latex_tab(json: &CallJson) -> LatexContent {
    let mut table = LatexTable::new();
    make_read_level_tab(&mut table, json, None);
    LatexContent::Table(table)
}

fn make_cpg_meth_profile_tab<T: Table>(
    table: &mut T,
    json: &CallJson,
    ms: Option<&mut MethSummary>,
) {
    ...
    for v in vrf.iter() {
        ...
        for (ix, x) in v.iter().enumerate() {
            y += x;
            if median.is_none() && y / z >= 0.5 {
                median = Some((ix as f64) * 0.01);
            }
            if ix < 30 {
                sums[0] += *x;
            } else if ix < 70 {
    ...
    table.add_row(vec![
        "m < 0.3".to_string(),
        format!("{:.0} ({:.2})", stats[0].2, f(stats[0].2, stats[0].1)),
    ...
    table.add_row(vec![
        "0.3 <= m < 0.7".to_string(),
        format!("{:.0} ({:.2})", stats[0].3, f(stats[0].3, stats[0].1)),
    ...
    table.add_row(vec![
        "m >= 0.7".to_string(),
        format!("{:.0} ({:.2})", stats[0].4, f(stats[0].4, stats[0].1)),
...
fn make_cpg_meth_profile_table(json: &CallJson, msumm: &mut MethSummary) -> Content {
    let mut table = HtmlTable::new("hor-zebra");
    make_cpg_meth_profile_tab(&mut table, json, Some(msumm));
    Content::Table(table)
}

fn make_cpg_meth_profile_latex_tab(json: &CallJson) -> LatexContent {
    let mut table = LatexTable::new();
...
fn make_variant_type_tab<T: Table>(
    table: &mut T,
    table1: &mut T,
    json: &CallJson,
    vs: Option<&mut VarSummary>,
) -> Result<(), String> {
    let mutations = json.mutations();
    let mut ct_ti = MutCounts::new();
    let mut ct_tv = MutCounts::new();
    let mut transitions = HashSet::new();
    for k in &["A>G", "G>A", "T>C", "C>T"] {
        transitions.insert(k);
    }
    let mut transversions = HashSet::new();
    for k in &["A>C", "A>T", "C>A", "C>G", "G>C", "G>T", "T>A", "T>G"] {
        transversions.insert(k);
    }
    ...
    for (k, c) in mutations.iter() {
        if transitions.contains(&(k.as_str())) {
            ...
            return Err(format!("Unknown variant type {} from CallJson file", k));
        }
    }
    let ct = ct_ti + ct_tv;
    ...
    if dbsnp {
        hdr.extend(&["dbSNP All", "%", "dbSNP Passed", "%"]);
    }
    ...
    let f = |k: &String, c: &MutCounts, s: &str| {
        let mut row = vec![
            ...
            s.to_string(),
            format!("{}", c.all()),
            format!("{:.2}", pct(c.all(), ct.all())),
            format!("{}", c.passed()),
            format!("{:.2}", pct(c.passed(), ct.passed())),
        ];
        if dbsnp {
            row.extend(vec![
                format!("{}", c.dbsnp_all()),
                format!("{:.2}", pct(c.dbsnp_all(), ct.dbsnp_all())),
                format!("{}", c.dbsnp_passed()),
                format!("{:.2}", pct(c.dbsnp_passed(), ct.dbsnp_passed())),
            ]);
        }
        row
    };
...
fn make_variant_type_tables(
    json: &CallJson,
    vsumm: &mut VarSummary,
...
fn make_vcf_filter_stats_tab<T: Table>(
    table: &mut T,
    json: &CallJson,
    vs: Option<&mut VarSummary>,
) -> Result<(), String> {
    ...
    let pass = if let Some(x) = pass {
        *x
    } else {
        return Err("No passed variant information found".to_string());
    };
    v.sort_by(|(_, a), (_, b)| (*b).all().cmp(&(*a).all()));
    let f = |s: &str, ct: QCCounts, tot: usize| {
        vec![
            s.to_owned(),
            format!("{}", ct.all()),
            format!("{:.2}", pct(ct.all(), tot)),
            format!("{}", ct.non_variant()),
            format!("{:.2}", pct(ct.non_variant(), tot)),
            format!("{}", ct.variant()),
            format!("{:.2}", pct(ct.variant(), tot)),
        ]
    };
...
fn create_mapping_report_body(
    project: &str,
    bc: &str,
    dir: &Path,
    json: &CallJson,
    msumm: &mut MapSummary,
) -> HtmlElement {
    ...
    table.add_row(vec![
        img_str(&get_path("coverage_all")),
        img_str(&get_path("quality_all")),
    ]);
    ...
    table.add_row(vec![
        img_str(&get_path("gc_coverage")),
        img_str(&get_path("non_cpg_read_profile")),
    ]);
    body.push(Content::Table(table));
    body
}

fn create_variant_report_body(
    project: &str,
    bc: &str,
    dir: &Path,
    json: &CallJson,
    ...
    let mut table = HtmlTable::new("green");
    table.add_header(vec![
        "Coverage Distribution for Variant Sites",
        "Quality Distribution for Variant Sites",
    ]);
    table.add_row(vec![
        img_str(&get_path("coverage_variants")),
        img_str(&get_path("quality_variants")),
    ]);
    body.push(Content::Table(table));
    body.push_element(HtmlElement::new("BR><BR><BR", None, false));
    ...
    table.add_header(vec![
        "Phred scale strand bias estimated by Fisher's Exact Test",
    ]);
    ...
    table = HtmlTable::new("green");
    table.add_header(vec![
        "Quality by Depth for Variant Alleles",
        "Quality by Depth for Non-Variant Alleles",
    ]);
    table.add_row(vec![
        img_str(&get_path("qd_variant")),
        img_str(&get_path("qd_nonvariant")),
    ]);
    body.push(Content::Table(table));
    body.push_element(HtmlElement::new("BR><BR><BR", None, false));
    table = HtmlTable::new("hor-zebra");
    table.add_header(vec![
        "RMS Mapping Quality for Variant Alleles",
        "RMS Mapping Qality for Non-Variant Alleles",
    ]);
    table.add_row(vec![
        img_str(&get_path("rmsmq_variant")),
        img_str(&get_path("rmsmq_nonvariant")),
    ]);
    body.push(Content::Table(table));
    body.push_element(HtmlElement::new("BR><BR><BR", None, false));
...
fn create_meth_report_body(
    project: &str,
    bc: &str,
    dir: &Path,
    json: &CallJson,
    msumm: &mut MethSummary,
) -> HtmlElement {
    let mut img_dir = dir.to_owned();
    img_dir.push("images");
    ...
    let mut table = HtmlTable::new("green");
    table.add_header(vec![
        "Coverage Distribution for Reference CpGs",
        "Informative Read Distribution for Reference CpGs",
    ]);
    table.add_row(vec![
        img_str(&get_path("coverage_ref_cpg")),
        img_str(&get_path("coverage_ref_cpg_inf")),
    ]);
    body.push(Content::Table(table));
    body.push_element(HtmlElement::new("BR><BR><BR", None, false));
    table = HtmlTable::new("hor-zebra");
    table.add_header(vec![
        "Quality Distribution for Reference CpGs",
        "Quality Distribution for Non Reference CpGs",
    ]);
    table.add_row(vec![
        img_str(&get_path("quality_ref_cpg")),
        img_str(&get_path("quality_non_ref_cpg")),
    ]);
    body.push_element(HtmlElement::new("BR><BR><BR", None, false));
...
fn create_mapping_latex_section(bc: &str, json: &CallJson) -> LatexSection {
    ...
    sec.push_string(format!(
        "\\subsection{{Alignment \\& Coverage Report for {}}}",
        bc
    ));
    ...
    sec.push_string(format!(
        ...
        img_dir
            .join(format!("{}_coverage_all", bc).as_str())
            .display()
    ));
    ...
    sec.push_string(format!(
        "\\includegraphics[width=12cm]{{{}}}",
        img_dir
            .join(format!("{}_quality_all", bc).as_str())
            .display()
    ));
    ...
    sec.push_string(format!(
        "\\includegraphics[width=12cm]{{{}}}",
        img_dir
            .join(format!("{}_gc_coverage", bc).as_str())
            .display()
    ));
    ...
    sec.push_string(format!(
        "\\includegraphics[width=12cm]{{{}}}",
        img_dir
            .join(format!("{}_non_cpg_read_profile", bc).as_str())
            .display()
    ));
    sec
}
...
fn create_variant_latex_section(bc: &str, json: &CallJson) -> Result<LatexSection, String> {
    ...
    let set_img = |nm: &str, sec: &mut LatexSection| {
        sec.push_string(format!(
            "\\includegraphics[width=12cm]{{{}}}",
            img_dir.join(format!("{}_{}", bc, nm).as_str()).display()
        ));
    };
...
fn create_meth_latex_section(bc: &str, json: &CallJson) -> LatexSection {
...
fn get_section_array_for_bc<'a>(
    ldoc: &'a mut LatexBare,
    bc: &str,
) -> Result<&'a mut SectionArray, String> {
...
fn create_mapping_report(
    bc: &str,
    dir: &Path,
    project: &str,
    call_json: &CallJson,
    summary: Arc<Mutex<HashMap<String, CallSummary>>>,
    latex_doc: Arc<Mutex<LatexBare>>,
) -> Result<(), String> {
    debug!("Create mapping report (from the calling stage) for {}", bc);
    let path: PathBuf = [
        dir,
        Path::new(format!("{}_mapping_coverage.html", bc).as_str()),
    ]
    .iter()
    .collect();
    let mut html = new_page(&path)?;
    let mut map_summ = MapSummary::new();
    html.push_element(create_mapping_report_body(
        project,
        bc,
        dir,
        call_json,
    ...
    shash
        .get_mut(&bc.to_owned())
        .expect("Couldn't find CallSummary for sample")
        .map = Some(map_summ);
...
fn create_variant_report(
    ...
    let path: PathBuf = [dir, Path::new(format!("{}_variants.html", bc).as_str())]
        .iter()
        .collect();
    let mut var_summ = VarSummary::new();
    let mut html = new_page(&path)?;
    html.push_element(create_variant_report_body(
        project,
        bc,
        dir,
        call_json,
...
fn create_meth_report(
    ...
    let path: PathBuf = [dir, Path::new(format!("{}_methylation.html", bc).as_str())]
        .iter()
        .collect();
    let mut html = new_page(&path)?;
    let mut meth_summ = MethSummary::new();
    html.push_element(create_meth_report_body(
        project,
        bc,
        dir,
        call_json,
...
fn create_summary(
    ...
    let f = |x| {
        if x > 1_000_000_000 {
            format!("{:.2} Gb", (x as f64) / 1_000_000_000.0)
        } else if x > 1_000_000 {
            format!("{:.2} Mb", (x as f64) / 1_000_000.0)
        } else if x > 1_000 {
            format!("{:.2} Kb", (x as f64) / 1_000.0)
        } else {
            format!("{}", x)
        }
    };
    ...
    ltable2.add_header(vec![
        "Variants",
        "Passed Variants",
        "Median Cov. of Passed Variants",
        "Ti/Tv Ratio",
    ]);
    ...
    if let Ok(sum_vec) = summary.lock() {
        for (bc, s) in sum_vec.iter() {
            ...
            row.push(format!(
                "{} ({:.2} %)",
                f(map_summ.unique),
                pct(map_summ.unique, map_summ.aligned)
            ));
            lrow1.push(format!(
                "{} ({:.2} %)",
                f(map_summ.unique),
            ...
            row.push(format!(
                "{} ({:.2} %)",
                f(map_summ.passed),
                pct(map_summ.passed, map_summ.aligned)
            ));
            lrow1.push(format!(
                "{} ({:.2} %)",
                f(map_summ.passed),
            ...
            row.push(format!(
                "{:.3e} ({:.2} %)",
                var_summ.variants_passed,
                pct(var_summ.variants_passed, var_summ.variants)
            ));
            lrow2.push(format!(
                "{:.3e} ({:.2} %)",
                var_summ.variants_passed,
            ...
            let mut link1 = HtmlElement::new(
                "a",
                Some(
                    format!(
                        "class=\"link\" href=\"{}/{}_mapping_coverage.html\"",
                        bc, bc
                    )
                    .as_str(),
                ),
            ...
            let mut link2 = HtmlElement::new(
                "a",
                Some(format!("class=\"link\" href=\"{}/{}_variants.html\"", bc, bc).as_str()),
                true,
            );
            link2.push_str("&#187 Variants");
            let mut link3 = HtmlElement::new(
                "a",
                Some(format!("class=\"link\" href=\"{}/{}_methylation.html\"", bc, bc).as_str()),
                true,
            );
    ...
    if let Ok(mut ldoc) = latex_doc.lock() {
        ldoc.push(LatexContent::Text(
            "\\section{{Sample Summaries}}".to_string(),
        ));
        ldoc.push(LatexContent::Text(
            "\\subsection{{Alignment \\& Coverage}}".to_string(),
        ));
        ldoc.push(LatexContent::Table(ltable1));
        ldoc.push(LatexContent::Text(
            "\\subsection{{Variant Calling}}".to_string(),
        ));
        ldoc.push(LatexContent::Table(ltable2));
        ldoc.push(LatexContent::Text(
            "\\subsection{{Methylation}}".to_string(),
        ));
...
fn make_call_job(bc: &str, bc_dir: &Path, project: &str, job: MakeCallJob) -> Result<(), String> {
    let t = job
        .call_json
        .read()
        .expect("Couldn't obtain read lock on CallJson structure");
    ...
    match job.job_type {
        ...
        CallJob::CoverageNonRefCpgInf => {
            make_coverage_graph(bc, &img_dir, CovType::NonRefCpgInf, cj)
        }
        ...
        CallJob::FsVariants => {
            let path: PathBuf = [
                &img_dir,
                Path::new(format!("{}_fs_variant.png", bc).as_str()),
            ]
            .iter()
            .collect();
            make_hist(
                &path,
                &cj.qc_dist().fisher_strand,
                "Fisher Strand Test",
                "Fisher Strand Phred Scale Probability",
                "# sites",
            )
            .map_err(|e| format!("{}", e))
        }
        CallJob::MappingReport => {
            create_mapping_report(bc, bc_dir, project, cj, job.summary, job.latex_doc)
        }
        CallJob::VariantReport => {
            create_variant_report(bc, bc_dir, project, cj, job.summary, job.latex_doc)
        }
        CallJob::MethylationReport => {
            create_meth_report(bc, bc_dir, project, cj, job.summary, job.latex_doc)
        }
    }
}
...
fn handle_call_job(job: ReportJob) -> Result<(), String> {
    match job.job {
        RepJob::CallJson(v) => load_call_json(v),
        RepJob::CallJob(v) => make_call_job(&job.barcode, &job.bc_dir, &job.project, v),
...
fn prepare_jobs(
    ...
    for cjson in svec.iter() {
        ...
        let load_json = LoadCallJson {
            path: cjson.json_file.clone(),
            call_json: call_json.clone(),
        };
        ...
        v.push(ReportJob::new(
            ...
            project,
            &cjson.bc_dir,
            RepJob::CallJson(load_json),
        ));
        for job_type in CallJob::iter() {
            let mk_graph = MakeCallJob {
                job_type,
                depend: ld_json_ix,
                call_json: call_json.clone(),
                summary: summary.clone(),
                latex_doc: latex_doc.clone(),
            };
            v.push(ReportJob::new(
                &cjson.barcode,
                project,
                &cjson.bc_dir,
                RepJob::CallJob(mk_graph),
            ));
        }
    }
...
pub fn make_call_report(
    sig: Arc<AtomicUsize>,
    outputs: &[PathBuf],
    project: Option<String>,
    css: &Path,
    n_cores: usize,
    svec: Vec<CallJsonFiles>,
) -> Result<(), String> {
    ...
    let output_dir = report_tex_path
        .parent()
        .expect("No parent directory found for call report");
    ...
    if abort {
        Err("Call report generation failed".to_string())
    } else {
