...
impl<T: AddAssign + Copy + Add<Output = T>> Reads<T> {
    pub fn get_total(&self) -> T {
        ...
        if let Some(x) = self.sequencing_control {
            ct += x
        }
        if let Some(x) = self.under_conversion_control {
            ct += x
        }
        if let Some(x) = self.over_conversion_control {
            ct += x
        }
        ct
    }
...
impl<T: AddAssign + Copy + Add<Output = T>> AddAssign for Reads<T> {
    fn add_assign(&mut self, other: Self) {
        ...
        self.under_conversion_control = add_option(
            self.under_conversion_control,
            other.under_conversion_control,
        );
        self.over_conversion_control =
            add_option(self.over_conversion_control, other.over_conversion_control);
    }
}
...
impl<T: Add<Output = T>> Add for NumReadsBS<T> {
    ...
    fn add(self, other: Self) -> Self {
        Self {
            c2t: self.c2t + other.c2t,
            g2a: self.g2a + other.g2a,
        }
    }
}
...
impl<T: AddAssign + Copy + Add<Output = T>> AddAssign for BaseCountStats<T> {
    fn add_assign(&mut self, other: BaseCountStats<T>) {
        ...
        self.under_conversion_control_c2t = add_option(
            self.under_conversion_control_c2t,
            other.under_conversion_control_c2t,
        );
        self.under_conversion_control_g2a = add_option(
            self.under_conversion_control_g2a,
            other.under_conversion_control_g2a,
        );
        self.over_conversion_control_c2t = add_option(
            self.over_conversion_control_c2t,
            other.over_conversion_control_c2t,
        );
        self.over_conversion_control_g2a = add_option(
            self.over_conversion_control_g2a,
            other.over_conversion_control_g2a,
        );
    }
}
...
impl Paired {
    fn merge(&mut self, other: &Self) {
        ...
        for i in 0..2 {
            for (key, ct) in other.hist_read_len[i].iter() {
                *(self.hist_read_len[i].entry(key.to_owned()).or_insert(0)) += ct;
            }
            for (key, ct) in other.hist_mismatch[i].iter() {
                *(self.hist_mismatch[i].entry(key.to_owned()).or_insert(0)) += ct;
            }
        }
        for (key, ct) in other.hist_template_len.iter() {
            *(self.hist_template_len.entry(key.to_owned()).or_insert(0)) += ct;
        }
    }
    pub fn reads(&self) -> Reads<Counts> {
        self.reads
    }
    pub fn bs_reads(&self) -> Option<NumReadsBS<Counts>> {
        self.num_reads_bs
    }
    pub fn base_counts(&self) -> BaseCountStats<Counts> {
        self.base_counts
    }
    pub fn correct_pairs(&self) -> usize {
        self.correct_pairs
    }
    pub fn read_len(&self) -> &[HashMap<String, usize>; 2] {
        &self.hist_read_len
    }
    pub fn mismatch(&self) -> &[HashMap<String, usize>; 2] {
        &self.hist_mismatch
    }
    pub fn template_len(&self) -> &HashMap<String, usize> {
        &self.hist_template_len
    }
}
...
impl Single {
    fn merge(&mut self, other: &Self) {
        ...
        self.base_counts += other.base_counts;
        add_assign_vec(&mut self.hist_mapq, &other.hist_mapq, 0);
        for (key, ct) in other.hist_read_len[0].iter() {
            *(self.hist_read_len[0].entry(key.to_owned()).or_insert(0)) += ct;
        }
        for (key, ct) in other.hist_mismatch[0].iter() {
            *(self.hist_mismatch[0].entry(key.to_owned()).or_insert(0)) += ct;
        }
    }
    pub fn reads(&self) -> Reads<Count> {
        self.reads
    }
    pub fn bs_reads(&self) -> Option<NumReadsBS<Count>> {
        self.num_reads_bs
    }
    pub fn base_counts(&self) -> BaseCountStats<Count> {
        self.base_counts
    }
    pub fn read_len(&self) -> &HashMap<String, usize> {
        &self.hist_read_len[0]
    }
    pub fn mismatch(&self) -> &HashMap<String, usize> {
        &self.hist_mismatch[0]
    }
}

#[derive(Clone, Copy, PartialEq)]
pub enum MapJsonType {
    Paired,
    Unknown,
    Single,
}
...
impl MapJson {
    ...
    pub fn get_conversion_counts(&self) -> (BaseCounts<Counts>, BaseCounts<Counts>) {
        ...
        match self {
            MapJson::Paired(s) | MapJson::Unknown(s) => {
                if let Some(bc) = s.base_counts.under_conversion_control_c2t {
                    ct1 = bc;
                }
                if let Some(bc) = s.base_counts.under_conversion_control_g2a {
                    ct1 += bc;
                }
                if let Some(bc) = s.base_counts.over_conversion_control_c2t {
                    ct2 = bc;
                }
                if let Some(bc) = s.base_counts.over_conversion_control_g2a {
                    ct2 += bc;
                }
            }
            MapJson::Single(s) => {
    ...
    pub fn merge(mut self, mut other: Self) -> Self {
        ...
        match (t1, t2) {
            ...
            (MapJsonType::Single, MapJsonType::Paired)
            | (MapJsonType::Single, MapJsonType::Unknown)
            | (MapJsonType::Paired, MapJsonType::Unknown) => {
    ...
    fn single_to_unknown(self) -> Self {
        if let MapJson::Single(s) = self {
            ...
            MapJson::Unknown(Paired {
                ...
                correct_pairs: 0,
                base_counts,
                hist_mapq: s.hist_mapq,
                hist_read_len,
                hist_mismatch,
                hist_template_len: HashMap::new(),
            })
        } else {
            panic!("Invalid conversion");
        }
