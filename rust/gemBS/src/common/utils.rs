...
use crate::common::defs::{SIGHUP, SIGINT, SIGQUIT, SIGTERM, signal_msg};
    ...
    S: AsRef<OsStr>,
{
    ...
    pub fn out_string_ref(&self) -> Option<&str> {
        if let PipelineOutput::String(Some(s)) = &self.output {
            Some(s)
        } else {
            None
        }
    ...
    pub fn run(&mut self, sig: Arc<AtomicUsize>) -> Result<(), String> {
        let log_file = if let Some(file) = &self.log {
            let f = fs::File::create(file).map_err(|e| {
                format!(
    ...
    fn do_run(&mut self, sig: Arc<AtomicUsize>, log: Option<fs::File>) -> Result<(), String> {
        if self.stage.is_empty() {
            return Err("Error - Empty pipeline".to_string());
        }
        ...
        for (com, args) in self.stage.drain(..) {
            ...
            if !arg_vec.is_empty() {
                cc = cc.args(arg_vec.iter())
            }
            let child = cc.spawn().map_err(|e| {
                format!(
                    "Error - problem launching command {}: {}",
                    com.to_string_lossy(),
                    e
                )
            })?;
            ...
            loop {
                if match child.try_wait() {
                    Ok(Some(st)) => {
                        if !st.success() {
                            err_com = Some(format!(
                                "Error from pipeline: {} exited with error",
                                com.to_string_lossy()
                            ))
                        }
                        true
...
fn get_lock_path(path: &Path, force: bool) -> Result<PathBuf, String> {
    ...
    let tfile = path
        .file_name()
        .ok_or(format!("Invalid file {:?} for LockedWriter::new()", path))?
        .to_string_lossy()
        .to_string();
    let file = if tfile.starts_with('.') {
        format!("{}#gemBS_lock", tfile)
    } else {
        format!(".{}#gemBS_lock", tfile)
    };
    ...
    if let Err(e) = symlink(Path::new(&lstring), &lock_path) {
        return match e.kind() {
            ErrorKind::AlreadyExists => {
                if let Ok(x) = fs::read_link(&lock_path) {
                    Err(format!("File locked by {}", x.to_string_lossy()))
                } else {
                    Err("File locked".to_string())
                }
            }
            _ => Err(format!("{}", e)),
        };
...
impl<'a> FileLock<'a> {
    ...
    pub fn pipe_writer<P: AsRef<Path>>(&self, prog: P) -> Result<Box<dyn Write>, String> {
        open_pipe_writer(self.path, prog.as_ref(), iter::empty::<&OsStr>()).map_err(|e| {
            format!(
                "Couldn't open pipe using {} for writing to {}: {}",
                prog.as_ref().display(),
                self.path.display(),
                e
            )
...
pub fn wait_for_lock(sig: Arc<AtomicUsize>, path: &Path) -> Result<FileLock<'_>, String> {
    timed_wait_for_lock(sig, path)
}
    ...
    loop {
            ...
            Err(e) => {
                if e.starts_with("File locked") {
                    if !message {
                        if signal == 0 {
                            warn!(
                                "Waiting for lock to allow clean shutdown: {}\nType Ctrl-C twice to quit",
                                e
                            );
                        } else {
                            warn!(
                                "Waiting for lock to allow clean shutdown: {}\nType Ctrl-C again to quit",
                                e
                            );
                        }
                        message = true;
        ...
        if s != 0 {
            if signal == 0 {
                ...
                return Err(format!("Received {} signal.  Closing down", signal_msg(s)));
            }
        }
        if let Ok(t) = now.elapsed()
            && t.as_secs() >= 30000
        {
            return Err("Timed out without obtaining lock".to_string());
        }
        thread::sleep(delay);
    }
}
