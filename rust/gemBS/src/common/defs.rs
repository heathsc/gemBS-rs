...
impl fmt::Display for MemSize {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        let mem = self.mem;
        if mem.trailing_zeros() >= 30 {
            write!(f, "{}G", mem >> 30)
        } else if mem.trailing_zeros() >= 20 {
            write!(f, "{}M", mem >> 20)
        } else if mem.trailing_zeros() >= 10 {
            write!(f, "{}K", mem >> 10)
        } else if f.alternate() {
            write!(f, "{}K", (mem + 0x3ff) >> 10)
        } else {
            write!(f, "{}", mem)
        }
    }
...
impl From<usize> for MemSize {
    fn from(mem: usize) -> Self {
        MemSize { mem }
    }
}

#[derive(Debug, Copy, Clone, PartialEq, Serialize, Deserialize)]
pub enum DbSnpFileType {
    Auto,
    Json,
    Bed,
    Vcf,
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        ...
        if let Some(cap) = RE.captures(s) {
            ...
            if let Some(s) = cap.get(1) {
                if let Ok(x) = <usize>::from_str(s.as_str().trim_end_matches('-')) {
                    v[0] = Some(x);
                } else {
            ...
            if let Some(s) = cap.get(4) {
                if let Ok(x) = <usize>::from_str(s.as_str()) {
                    v[3] = Some(x);
                } else {
                    return Err("Invalid time");
                }
            ...
            match v[..] {
                ...
                [None, Some(mins), None, Some(secs)] if mins <= 60 && secs <= 60 => Ok(JobLen {
                    secs: mins * 60 + secs,
                }),
                [None, Some(hrs), Some(mins), Some(secs)]
                ...
                [Some(days), None, None, Some(hrs)] if hrs <= 24 => Ok(JobLen {
                    secs: (days * 24 + hrs) * 3600,
                }),
                [Some(days), Some(hrs), None, Some(mins)] if hrs <= 24 && mins <= 60 => {
                    Ok(JobLen {
                        secs: (days * 24 * 60 + hrs * 60 + mins) * 60,
                    })
                }
                [Some(days), Some(hrs), Some(mins), Some(secs)]
                    if hrs <= 24 && mins <= 60 && secs <= 60 =>
                {
                    Ok(JobLen {
                        secs: days * 24 * 3600 + hrs * 3600 + mins * 60 + secs,
                    })
                }
                _ => Err("Invalid time"),
            }
