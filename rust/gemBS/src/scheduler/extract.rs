...
fn make_contig_file(gem_bs: &GemBS, barcode: &str, output_dir: &Path) -> PathBuf {
    let ctg_path: PathBuf = [
        output_dir,
        Path::new(format!("{}_mextr_ctgs.bed", barcode).as_str()),
    ]
    .iter()
    .collect();
    ...
    let omit_hash = {
        if let Some(oc) = gem_bs.get_config_stringvec(Section::Index, "omit_ctgs") {
            oc.iter().fold(HashSet::new(), |mut h, x| {
                h.insert(x.clone());
                h
            })
        } else {
    ...
    for ctg in vr_ctg
        .iter()
        .filter(|x| !omit_hash.contains(x.name.as_str()))
    {
...
fn make_mextr_pipeline(gem_bs: &GemBS, job: usize, bc: &str) -> QPipe {
    ...
    let first_output = gem_bs
        .get_asset(
            *task
                .outputs()
                .next()
                .expect("No output files for extract step"),
        )
    ...
    // Set up arg list
    let mut args = format!(
        "--loglevel\x1e{}\x1e--compress\x1e--md5\x1e--regions-file\x1e{}\x1e",
        gem_bs.verbose(),
        contig_file.to_string_lossy()
    );
    ...
    if noncpg {
        opt_list.push(("min_nc", "min-nc", VarType::Int));
    }
    if cpg {
        opt_list.push(("strand_specific", "mode\x1estrand-specific", VarType::Bool));
    }
    if bedmethyl {
        opt_list.push((
            "bigwig_strand_specific",
            "bw-mode\x1estrand-specific",
            VarType::Bool,
        ));
    }
    ...
    for out in task.outputs() {
        pipeline.add_outputs(
            gem_bs
                .get_asset(*out)
                .expect("Couldn't get mextr output asset")
                .path(),
        );
    }
    if gem_bs.keep_logs() || gem_bs.get_config_bool(Section::Extract, "keep_logs") {
        pipeline.set_remove_log(false)
    }
    pipeline
        .add_stage(&mextr_path, &args)
        .add_remove_file(&contig_file);
    pipeline
}

fn make_snpxtr_pipeline(gem_bs: &GemBS, job: usize) -> QPipe {
    let task = &gem_bs.get_tasks()[job];
    let first_out = gem_bs
        .get_asset(
            *task
                .outputs()
                .next()
    ...
    // Set up arg list
    let mut args = format!(
        "--loglevel\x1e{}\x1e--compress\x1e--md5\x1e--tabix\x1e--output\x1e{}\x1e",
        gem_bs.verbose(),
    ...
    for out in task.outputs() {
        pipeline.add_outputs(
            gem_bs
                .get_asset(*out)
                .expect("Couldn't get snpxtr output asset")
                .path(),
        );
    }
    if gem_bs.keep_logs() || gem_bs.get_config_bool(Section::Extract, "keep_logs") {
...
fn get_command_and_barcode(id: &str) -> (&str, &str) {
    lazy_static! {
        static ref RE: Regex = Regex::new(r"^(mextr|snpxtr)_(.*)$").unwrap();
    }
    if let Some(cap) = RE.captures(id) {
        (cap.get(1).unwrap().as_str(), cap.get(2).unwrap().as_str())
    } else {
        panic!("Couldn't parse extract task id")
    }
}
