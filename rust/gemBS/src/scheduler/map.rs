...
use crate::common::defs::{DataValue, FileType, Metadata, Section, VarType};
...
fn check_inputs<'a>(gem_bs: &'a GemBS, task: &'a Task) -> (Vec<&'a Asset>, &'a str) {
    ...
    for ix in task.inputs() {
        ...
        if let Some(cap) = REFILE.captures(asset.id()) {
            ...
            files[x] = Some(ix);
            if let Some(dat) = cap.get(1) {
                dataset = Some(dat.as_str());
            }
    ...
    let index = gem_bs
        .get_asset(*files[0].expect("No index file found for task"))
        .unwrap();
    let mut vfile = Vec::new();
    if let Some(f) = files[3] {
        vfile.push(gem_bs.get_asset(*f).unwrap())
    } else {
        if let Some(f) = files[1] {
            vfile.push(gem_bs.get_asset(*f).unwrap())
        }
        if let Some(f) = files[2] {
            vfile.push(gem_bs.get_asset(*f).unwrap())
        }
    }
    if vfile.is_empty() {
        panic!("No datafiles found for task")
    };
    vfile.push(index);
    (vfile, dataset)
}
...
fn check_outputs<'a>(gem_bs: &'a GemBS, task: &'a Task) -> [Option<&'a Asset>; 3] {
    ...
    if ofiles[2].is_none() || (ofiles[0].is_none() && ofiles[1].is_none()) {
        panic!("Missing output files!");
    }
    ofiles
}

fn get_read_groups(dataset: &str, href: &HashMap<Metadata, DataValue>) -> String {
    let sample = if let Some(DataValue::String(x)) = href.get(&Metadata::SampleName) {
        x
    } else {
        ""
    };
    let barcode = if let Some(DataValue::String(x)) = href.get(&Metadata::SampleBarcode) {
        x
    } else {
        ""
    };
    let mut read_groups = format!(
        "@RG\\tID:{}\\tSM:{}\\tBC:{}\\tPU:{}",
        dataset, sample, barcode, dataset
    );
    for (tp, nm) in &[
        (Metadata::Description, "DS"),
        (Metadata::LibraryBarcode, "LB"),
        (Metadata::Centre, "CN"),
        (Metadata::Platform, "PL"),
    ] {
        if let Some(DataValue::String(x)) = href.get(tp) {
            read_groups.push_str(format!("\\t{}:{}", nm, x).as_str())
        }
    }
    read_groups
}
...
pub fn make_map_pipeline(
    ...
    let mapping_threads = gem_bs
        ...
        .or(threads);
    let sort_threads = gem_bs
        .get_config_int(Section::Mapping, "sort_threads")
        .or(mapping_threads);
    let mut pipeline = QPipe::new(gem_bs.get_signal_clone());
    let mapper_path = gem_bs.get_exec_path("gem-mapper");
    let mut mapper_args = if let Some(t) = mapping_threads {
        format!("--threads\x1e{}\x1e", t)
    } else {
        String::new()
    };
    ...
    let (outfile, cram) = if let Some(x) = outs[0] {
        (x, false)
    } else if let Some(x) = outs[1] {
        (x, true)
    } else {
        panic!("No mapping outfile set!")
    };
    ...
    let href = gem_bs
        .get_sample_data_ref()
        .get(dataset)
        .unwrap_or_else(|| panic!("No sample data for dataset {}", dataset));
    // Set read_groups
    let read_groups = get_read_groups(dataset, href);

    // Setup rest of arguments for mapper
    let ftype = if let Some(DataValue::FileType(t)) = href.get(&Metadata::FileType) {
        Some(*t)
    } else {
        None
    };
    ...
    if vfile.len() == 2 {
        mapper_args.push_str(
            format!(
                "--i1\x1e{}\x1e--i2\x1e{}\x1e",
                vfile[0].path().display(),
                vfile[1].path().display()
            )
            .as_str(),
        );
    } else if let Some(FileType::BAM) = ftype {
        let bam2fq = gem_bs.get_exec_path("samtools");
        let args = if let Some(t) = mapping_threads {
            format!(
                "bam2fq\x1e{}\x1e--threads\x1e{}",
                vfile[0].path().display(),
                t
            )
        } else {
            format!("bam2fq {}", vfile[0].path().display())
        };
        pipeline.add_stage(&bam2fq, &args);
    } else {
        mapper_args.push_str(format!("-i\x1e{}\x1e", vfile[0].path().display()).as_str())
    }
    if paired {
        mapper_args.push_str("--paired-end-alignment\x1e")
    }
    if gem_bs.get_config_bool(Section::Mapping, "non_stranded") {
        mapper_args.push_str("--bisulfite-conversion\x1enon-stranded\x1e")
    } else if gem_bs.get_config_bool(Section::Mapping, "reverse_conversion") {
        mapper_args.push_str("--bisulfite-conversion\x1einferred-G2A-C2T\x1e")
    } else {
    ...
    let contig_md5 = gem_bs
        .get_asset("contig_md5")
        .expect("Couldn't find contig md5 asset");
    ...
    if let Some(x) = tmp_dir {
        samtools_args.push_str(format!("-T\x1e{}\x1e", x.display()).as_str())
    }
    if let Some(x) = gem_bs.get_config_str(Section::Mapping, "sort_memory") {
        samtools_args.push_str(format!("-m\x1e{}\x1e", x).as_str())
    }
    if let Some(x) = sort_threads {
        samtools_args.push_str(format!("--threads\x1e{}\x1e", x).as_str())
    }
    if single_bam {
        samtools_args.push_str("--write-index\x1e")
    }
    if cram {
        samtools_args.push_str("-O\x1eCRAM\x1e")
    }
    if gem_bs.get_config_bool(Section::Mapping, "benchmark_mode") {
        samtools_args.push_str("--no-PG\x1e")
    } else if cram {
        let gembs_ref = gem_bs
            .get_asset("gembs_reference")
            .expect("Couldn't find gemBS reference asset");
        samtools_args
            .push_str(format!("--reference\x1e{}\x1e", gembs_ref.path().display()).as_str());
    }
    samtools_args.push('-');
    ...
    pipeline
        .add_stage(&mapper_path, &mapper_args)
    ...
    let output = gem_bs
        .get_asset(
            *task
                .outputs()
                .next()
                .expect("No output files for merge step"),
        )
        .expect("Couldn't get asset");
    let cram = output.id().ends_with(".cram");
    if cram {
        args.push_str("-O\x1ecram\x1e")
    }
    if gem_bs.get_config_bool(Section::Mapping, "benchmark_mode") {
        args.push_str("--no-PG\x1e")
    } else if cram {
        let gembs_ref = gem_bs
            .get_asset("gembs_reference")
            .expect("Couldn't find gemBS reference asset");
