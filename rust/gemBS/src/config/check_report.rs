...
pub fn check_map_report(gem_bs: &mut GemBS) -> Result<(), String> {
    let get_dir = |name: &str| {
        if let Some(DataValue::String(x)) = gem_bs.get_config(Section::Report, name) {
            x
        } else {
            "gemBS_reports"
        }
    ...
    for (bc, _) in samples.iter() {
        ...
        let img_dir: PathBuf = [&report_dir, Path::new(bc), Path::new("images")]
            .iter()
            .collect();
        out_vec.push(handle_file(
            gem_bs,
            format!("{}_map_index.html", bc).as_str(),
            format!("{}.html", bc).as_str(),
            &bc_dir,
        ));
        out_vec.push(handle_file(
            gem_bs,
            format!("{}_isize.png", bc).as_str(),
            format!("{}_isize.png", bc).as_str(),
            &img_dir,
        ));
        out_vec.push(handle_file(
            gem_bs,
            format!("{}_mapq.png", bc).as_str(),
            format!("{}_mapq.png", bc).as_str(),
            &img_dir,
        ));
    ...
    for (bc, dset) in dsets.iter() {
        if *bc_count.get(bc).expect("No count found for barcode") > 1 {
            ...
            out_vec.push(handle_file(
                gem_bs,
                format!("{}.html", dset).as_str(),
                format!("{}.html", dset).as_str(),
                &bc_dir,
            ));
            out_vec.push(handle_file(
                gem_bs,
                format!("{}_mapq.png", dset).as_str(),
                format!("{}_mapq.png", dset).as_str(),
                &img_dir,
            ));
            out_vec.push(handle_file(
                gem_bs,
                format!("{}_isize.png", dset).as_str(),
                format!("{}_isize.png", dset).as_str(),
                &img_dir,
            ));
        }
...
pub fn check_call_report(gem_bs: &mut GemBS) -> Result<(), String> {
    ...
    for (bc, _) in samples.iter() {
        ...
        for name in &[
            "coverage_all",
            "coverage_ref_cpg",
            "coverage_ref_cpg_inf",
            "coverage_non_ref_cpg",
            "coverage_non_ref_cpg_inf",
            "coverage_variants",
            "quality_all",
            "quality_ref_cpg",
            "quality_non_ref_cpg",
...
pub fn check_report(gem_bs: &mut GemBS) -> Result<(), String> {
    ...
    let in_vec = [
        gem_bs
            .get_asset("map_report_index.html")
            .expect("Couldn't find map report index asset")
            .idx(),
        gem_bs
            .get_asset("map_report.tex")
            .expect("Couldn't find map report latex asset")
            .idx(),
        gem_bs
            .get_asset("call_report_index.html")
            .expect("Couldn't find call report index asset")
            .idx(),
        gem_bs
            .get_asset("call_report.tex")
            .expect("Couldn't find call report latex asset")
            .idx(),
    ];
    ...
    let mut out_vec = Vec::new();
    let name = if let Some(s) = project {
        format!("{}_QC_Report", s)
    } else {
        "GemBS_QC_Report".to_string()
    };
    out_vec.push(handle_file(
        gem_bs,
        "report.tex",
