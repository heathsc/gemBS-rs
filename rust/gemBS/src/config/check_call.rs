...
pub fn check_call(gem_bs: &mut GemBS) -> Result<(), String> {
    let get_dir = |name: &str| {
        if let Some(DataValue::String(x)) = gem_bs.get_config(Section::Calling, name) {
            x
        } else {
            "."
        }
    ...
    let merge_cores = gem_bs
        ...
        .or(cores);
    let memory = gem_bs
        .get_config_memsize(Section::Calling, "memory")
        .or_else(|| Some(0x100000000.into())); // 4G
    let merge_memory = gem_bs
        .get_config_memsize(Section::Calling, "merge_memory")
        .or(memory);
    let time = gem_bs
        .get_config_joblen(Section::Calling, "time")
    ...
    if let Some(x) = gem_bs.get_asset("dbsnp_index") {
        common_inputs.push(x.idx())
    }
    let handle_file = |gb: &mut GemBS, nm: String, id: Option<String>, p: &Path, st: AssetType| {
        ...
        if let Some(s) = id {
            gb.insert_asset(s, &path, st)
        } else {
            gb.insert_asset(nm, &path, st)
        }
    };
    for (bcode, name) in samples.iter() {
        let bam = if let Some(x) = gem_bs.get_asset(format!("{}.{}", bcode, ext).as_str()) {
            x.idx()
        } else {
            panic!("alignment file {}.{} not found", bcode, ext);
        };
        let bam_idx = if let Some(x) = gem_bs.get_asset(format!("{}.{}", bcode, idx_ext).as_str()) {
            x.idx()
        } else {
            panic!("index file {}.{} not found", bcode, idx_ext);
        };
        let bam_md5 = if let Some(x) = gem_bs.get_asset(format!("{}.{}.md5", bcode, ext).as_str()) {
            x.idx()
        } else {
            panic!("alignment md5 file {}.{}.md5 not found", bcode, ext);
        };
        let replace_meta_var = |s: &str| {
            if let Some(sm) = name {
                s.replace("@BARCODE", bcode).replace("@SAMPLE", sm)
            } else {
                s.replace("@BARCODE", bcode)
            }
        };
        ...
        let bcf_asset = gem_bs
            .get_asset(id.as_str())
            .expect("Couldn't find bcf asset")
            .idx();
        ...
        let indexed = if pools.len() > 1 {
            ...
            for pool in pools.iter() {
                ...
                let out1 = handle_file(
                    gem_bs,
                    format!("{}_{}.json", bcode, pool),
                    Some(format!("{}_{}_call.json", bcode, pool)),
                    bcf_path,
                    AssetType::Temp,
                );
                out_bcfs.push(out);
                ...
                let call_task = gem_bs.add_task(
                    &id,
                    format!("Call BCFs for pool {}, barcode {}", pool, bcode).as_str(),
                    Command::Call,
                    format!("--barcode {} --pool {}", bcode, pool).as_str(),
                );
                gem_bs
                    .add_task_inputs(call_task, &in_vec)
                    .add_outputs(&[out, out1])
            ...
            out_jsons[0] = out; // Add bcf file as input requirement for JSON merge so that we don't do the merge step until the merge BCF step is complete
            let out1 = handle_file(
                gem_bs,
                format!("{}_call.json", bcode),
                None,
                bcf_path,
                AssetType::Derived,
            );
            ...
            let (lname, lpath) = assets::make_ext_asset(&id, bcf_path, "log");
            let log_index = gem_bs.insert_asset(&lname, &lpath, AssetType::Log);
            let merge_task = gem_bs.add_task(
                &id,
                format!("Merge BCFs for barcode {}", bcode).as_str(),
                Command::MergeBcfs,
                format!("--barcode {} --no-md5", bcode).as_str(),
            );
            gem_bs
                .add_task_inputs(merge_task, &out_bcfs)
                .add_outputs(&[out, csi])
                .set_log(Some(log_index))
                .set_barcode(bcode)
            ...
            gem_bs
                .get_asset_mut(out)
                .unwrap()
                .set_creator(merge_task, &out_bcfs);
            let id = format!("merge_call_jsons_{}", bcode);
            let merge_json_task = gem_bs.add_task(
                &id,
                format!("Merge JSONs for barcode {}", bcode).as_str(),
                Command::MergeCallJsons,
                "",
            );
            gem_bs
                .add_task_inputs(merge_json_task, &out_jsons)
                .add_outputs(&[out1])
                .set_barcode(bcode);
            gem_bs
                .get_asset_mut(out1)
            ...
            let call_task = gem_bs.add_task(
                ...
                Command::Call,
                format!("--barcode {} --no-md5 --no-index", bcode).as_str(),
            );
            gem_bs
                .add_task_inputs(call_task, &in_vec)
                .add_outputs(&[out, out1])
        ...
        let (md5_name, md5_path) =
            assets::make_ext_asset(gem_bs.get_asset(bcf_asset).unwrap().id(), bcf_path, "md5");
        ...
        let md5_time = gem_bs
            ...
            .or_else(|| Some(3600.into()));
        gem_bs
            .add_task_inputs(md5_task, &[bcf_asset])
            .add_outputs(&[md5])
            .set_barcode(bcode)
            .add_cores(md5_cores)
        ...
        gem_bs
            .get_asset_mut(md5)
            .unwrap()
            .set_creator(md5_task, &[bcf_asset]);

        if !indexed {

            let csi_task = gem_bs.add_task(
                &csi_name,
                format!("Calc Index for {}", id).as_str(),
                Command::IndexBcf,
                format!("--barcode {}", bcode).as_str(),
            );
            gem_bs
                .add_task_inputs(csi_task, &[bcf_asset])
                .add_outputs(&[csi])
                .set_barcode(bcode)
                .add_cores(cores)
                .add_memory(memory)
            ...
            gem_bs
                .get_asset_mut(csi)
                .unwrap()
                .set_creator(csi_task, &[bcf_asset]);
        }
    }
    Ok(())
}
