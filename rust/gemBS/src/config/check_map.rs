...
use crate::common::defs::{Command, DataValue, FileType, Metadata, Section};
        ...
        if let Some(name1) = name {
            match self.name {
                Some(name2) => {
                    if name1 != name2 {
                        return Err(format!(
                            "Inconsistent sample name for barcode {}: found {}, expected {}",
                            barcode, name1, name2
                        ));
                    }
                }
...
fn collect_samples(gem_bs: &GemBS) -> Result<Vec<Sample>, String> {
    ...
    for (dataset, href1) in href.iter() {
        if let Some(DataValue::String(bcode)) = href1.get(&Metadata::SampleBarcode) {
            ...
            let sample = bc.entry(bcode).or_insert_with(|| TempSample {
                name,
                datasets: Vec::new(),
            });
            ...
            return Err(format!("No barcode associated with dataset {}", dataset));
        }
    }
...
fn check_file_match(path: &Path) -> Option<Metadata> {
    lazy_static! {
        static ref RE: Regex = Regex::new(r"^.*?([12])?[.]fastq").unwrap();
    }
    if let Some(s) = path.as_os_str().to_str() {
        if let Some(cap) = RE.captures(s) {
            if let Some(r) = cap.get(1) {
                if r.as_str() == "1" {
                    Some(Metadata::FilePath1)
                } else {
                    Some(Metadata::FilePath2)
                }
            } else {
                Some(Metadata::FilePath)
            }
        } else {
            None
        }
...
pub fn check_map(gem_bs: &mut GemBS) -> Result<(), String> {
    let get_dir = |name: &str| {
        if let Some(DataValue::String(x)) = gem_bs.get_config(Section::Mapping, name) {
            x
        } else {
            "."
        }
    ...
    let merge_memory = gem_bs
        .get_config_memsize(Section::Mapping, "merge_memory")
        .or(memory);
    let time = gem_bs
        .get_config_joblen(Section::Mapping, "time")
        .or_else(|| Some(21600.into()));
    let merge_time = gem_bs
        .get_config_joblen(Section::Mapping, "merge_time")
        .or(time);
    ...
    if make_cram {
        for f in &[
            ...
            if let Some(x) = gem_bs.get_asset(*f) {
                common_inputs.push(x.idx())
            } else {
                panic!("{} not found", f)
            };
        }
    ...
    let (suffix, index_suff) = if make_cram {
        (".cram", ".cram.crai")
    } else {
        (".bam", ".bam.csi")
    };
    ...
    let handle_file = |gb: &mut GemBS, dt: &str, s: &str, nm: &str, p: &Path, asset: AssetType| {
        ...
        let path = if tpath.has_root() || shell {
            tpath.to_owned()
        } else {
            [p, tpath].iter().collect()
        };
    ...
    for sample in samples.iter() {
        let replace_meta_var = |s: &str| {
            if let Some(sm) = &sample.name {
                s.replace("@BARCODE", &sample.barcode)
                    .replace("@SAMPLE", sm.as_str())
            } else {
                s.replace("@BARCODE", &sample.barcode)
            }
        };
        ...
        for dat in sample.datasets.iter() {
            ...
            let dvec: Vec<(Metadata, DataValue)> = if let Some(dr) = href.get(dat) {
                ...
                panic!("Lost dataset information {}", dat)
            };
            ...
            for (md, val) in dvec.iter() {
                ...
                match md {
                    Metadata::FilePath => {
                        if let DataValue::String(s) = val {
                            in_vec.push(handle_file(gem_bs, dat, s, "_read", spath, asset))
                        }
                    }
                    Metadata::FilePath1 => {
                        if let DataValue::String(s) = val {
                            in_vec.push(handle_file(gem_bs, dat, s, "_read1", spath, asset))
                        }
                    }
                    Metadata::FilePath2 => {
                        if let DataValue::String(s) = val {
                            in_vec.push(handle_file(gem_bs, dat, s, "_read2", spath, asset))
                        }
                    }
                    Metadata::AltDataset => {
                        if let DataValue::String(s) = val {
                    ...
                    Metadata::FileType => {
                        if let DataValue::FileType(t) = val {
                            file_type = Some(t)
                        }
                    }
                    Metadata::Bisulfite => {
                        if let DataValue::Bool(x) = val
                            && !*x
                        {
                            bisulfite = false;
                        }
            ...
            if in_vec.is_empty() {
                ...
                if let Some(d) = alt_dataset {
                    dsets.push(d);
                }
                for dt in dsets {
                    for mat in glob(format!("{}/*{}*fastq*", sdir, dt).as_str())
                        .expect("Failed to read glob pattern")
                    {
                        match mat {
                            Ok(p) => {
                                if let Some(md) = check_file_match(&p)
                                    && let Some(old_val) = thash.insert(md, p.clone())
                                    && old_val != p
                                {
                                    return Err(format!(
                            ...
                            Err(e) => {
                                return Err(format!("Error when searching for datafiles: {}", e));
                            }
                        }
                    }
                    if !thash.is_empty() {
                ...
                if thash.contains_key(&Metadata::FilePath1)
                    || thash.contains_key(&Metadata::FilePath2)
                {
                    if thash.contains_key(&Metadata::FilePath) {
                        return Err(format!(
                            "Mixture of paired and single sequence files found for dataset {} in {}",
                            dat, sdir
                        ));
                    }
                    if thash.contains_key(&Metadata::FilePath1)
                        && thash.contains_key(&Metadata::FilePath2)
                    {
                        if let Some(ft) = file_type {
                            if *ft == FileType::Interleaved || *ft == FileType::Single {
                                return Err(format!(
                                    "Error with dataset {}: Interleaved or Single file type incompatible with multiple sequence files",
                                    dat
                                ));
                            }
                        } else {
                            gem_bs.set_sample_data(
                                dat,
                                Metadata::FileType,
                                DataValue::FileType(FileType::Paired),
                            )
                        }
                    }
                }
                ...
                for (md, p) in thash.iter() {
                    ...
                    gem_bs.set_sample_data(
                        dat,
                        *md,
                        DataValue::String(format!("{}", p_abs.display())),
                    );
            ...
            if in_vec.is_empty() {
                return Err(format!("No datafiles for dataset {}", dat));
            }
            if bisulfite {
                in_vec.push(index_id.unwrap());
            } else {
                in_vec.push(nonbs_index_id.unwrap());
            }
            ...
            if sample.datasets.len() > 1 {
                let out1 = handle_file(
                    gem_bs,
                    dat,
                    format!("{}.bam", dat).as_str(),
                    ".bam",
                    bpath,
                    AssetType::Temp,
                );
                let out2 = handle_file(
                    gem_bs,
                    dat,
                    format!("{}.json", dat).as_str(),
                    "_map.json",
                    bpath,
                    AssetType::Derived,
                );
                ...
                let task = gem_bs.add_task(
                    &id,
                    format!("Map dataset {} for barcode {}", dat, sample.barcode).as_str(),
                    Command::Map,
                    format!("--dataset {}", dat).as_str(),
                );
                gem_bs
                    .add_task_inputs(task, &in_vec)
                    .add_outputs(&[out1, out2])
                ...
                let out1 = handle_file(
                    gem_bs,
                    &sample.barcode,
                    format!("{}{}", sample.barcode, suffix).as_str(),
                    suffix,
                    bpath,
                    AssetType::Derived,
                );
                let out2 = handle_file(
                    gem_bs,
                    &sample.barcode,
                    format!("{}.json", sample.barcode).as_str(),
                    "_map.json",
                    bpath,
                    AssetType::Derived,
                );
                let out3 = handle_file(
                    gem_bs,
                    &sample.barcode,
                    format!("{}{}", sample.barcode, index_suff).as_str(),
                    index_suff,
                    bpath,
                    AssetType::Derived,
                );
                ...
                let task = gem_bs.add_task(
                    ...
                    format!("Map dataset {} for barcode {}", dat, sample.barcode).as_str(),
                    Command::Map,
                    format!("--barcode {} --no-md5", sample.barcode).as_str(),
                );
                gem_bs
                    .add_task_inputs(task, &in_vec)
                    .add_outputs(&[out1, out2, out3])
                    .set_log(Some(log_index))
                    .set_barcode(&sample.barcode)
                    .add_cores(cores)
                    .add_memory(memory)
                ...
                let (md5_name, md5_path) =
                    assets::make_ext_asset(gem_bs.get_asset_mut(out1).unwrap().id(), bpath, "md5");
                let md5 = gem_bs.insert_asset(&md5_name, &md5_path, AssetType::Derived);
                let md5_task = gem_bs.add_task(
                    &md5_name,
                    format!("Calc MD5 sum for {}", id).as_str(),
                    Command::MD5SumMap,
                    format!("--barcode {}", sample.barcode).as_str(),
                );
                let md5_cores = gem_bs
                ...
                let md5_time = gem_bs
                    ...
                    .or_else(|| Some(3600.into()));
                gem_bs
                    .add_task_inputs(md5_task, &[out1])
                    .add_outputs(&[md5])
                    .set_barcode(&sample.barcode)
                    .add_cores(md5_cores)
        ...
        if !bams.is_empty() {
            ...
            let task = gem_bs.add_task(
                &id,
                format!("Merge datasets for barcode {}", sample.barcode).as_str(),
                Command::MergeBams,
                format!("--barcode {} --no-md5", sample.barcode).as_str(),
            );
            gem_bs
                .add_task_inputs(task, &bams)
                .add_outputs(&[out1, out2])
                .set_log(Some(log_index))
                .set_barcode(&sample.barcode)
            ...
            [out1, out2]
                .iter()
                .for_each(|id| gem_bs.get_asset_mut(*id).unwrap().set_creator(task, &bams));
