...
impl GemBS {
    ...
    pub fn get_signal_clone(&self) -> Arc<AtomicUsize> {
        Arc::clone(&self.signal)
    }
    ...
    fn get_contigs_mut(&mut self) -> &mut Vec<Contig> {
        if let GemBSData::Contigs(vref) = &mut self.var[2] {
            vref
        } else {
            panic!("Internal error!");
        }
    ...
    pub fn get_contigs(&self) -> &Vec<Contig> {
        if let GemBSData::Contigs(vref) = &self.var[2] {
            vref
        } else {
            panic!("Internal error!");
        }
    ...
    pub fn get_contig_pool_hash(&self) -> &HashMap<Rc<String>, ContigPool> {
        if let GemBSData::ContigPools(href) = &self.var[3] {
            href
        } else {
            panic!("Internal error!");
        }
    ...
    fn get_contig_pool_hash_mut(&mut self) -> &mut HashMap<Rc<String>, ContigPool> {
        if let GemBSData::ContigPools(href) = &mut self.var[3] {
            href
        } else {
            panic!("Internal error!");
        }
    ...
    pub fn get_config_bool(&self, section: Section, name: &str) -> bool {
        if let Some(DataValue::Bool(x)) = self.get_config(section, name) {
            *x
        } else {
            false
        }
    ...
    pub fn get_config_int(&self, section: Section, name: &str) -> Option<isize> {
        if let Some(DataValue::Int(x)) = self.get_config(section, name) {
            Some(*x)
        } else {
            None
        }
    ...
    pub fn get_config_intvec(&self, section: Section, name: &str) -> Option<&Vec<isize>> {
        if let Some(DataValue::IntVec(x)) = self.get_config(section, name) {
            Some(x)
        } else {
            None
        }
    ...
    pub fn get_config_str(&self, section: Section, name: &str) -> Option<&str> {
        if let Some(DataValue::String(x)) = self.get_config(section, name) {
            Some(x)
        } else {
            None
        }
    ...
    pub fn get_config_stringvec(&self, section: Section, name: &str) -> Option<&Vec<String>> {
        if let Some(DataValue::StringVec(x)) = self.get_config(section, name) {
            Some(x)
        } else {
            None
        }
    ...
    pub fn get_config_float(&self, section: Section, name: &str) -> Option<f64> {
        if let Some(DataValue::Float(x)) = self.get_config(section, name) {
            Some(*x)
        } else {
            None
        }
    ...
    pub fn get_config_joblen(&self, section: Section, name: &str) -> Option<JobLen> {
        if let Some(DataValue::JobLen(x)) = self.get_config(section, name) {
            Some(*x)
        } else {
            None
        }
    ...
    pub fn get_config_memsize(&self, section: Section, name: &str) -> Option<MemSize> {
        if let Some(DataValue::MemSize(x)) = self.get_config(section, name) {
            Some(*x)
        } else {
            None
        }
    ...
    pub fn insert_asset<P: AsRef<Path>, S: AsRef<str>>(
        &mut self,
        id: S,
        path: P,
    ...
    pub fn add_task_inputs(&mut self, task: usize, inputs: &[usize]) -> &mut Task {
        for inp in inputs.iter() {
            if let Some(x) = self.assets.get_asset(*inp).unwrap().creator() {
                self.add_parent_child(task, x);
            }
        }
    ...
    pub fn add_task(&mut self, id: &str, desc: &str, command: Command, args: &str) -> usize {
        ...
        self.tasks.add_task(id, command, args)
    }
    pub fn get_tasks(&self) -> &TaskList {
        &self.tasks
    }
    pub fn get_assets(&self) -> &AssetList {
        &self.assets
    }
    ...
    pub fn write_config(&self) -> Result<(), String> {
        ...
        let lock = FileLock::new(config_file)
            .map_err(|e| format!("Error: Could not obtain lock on gemBS config file: {}", e))?;
        let mut writer = {
            ...
            .map_err(|e| {
                format!(
                    "Error: Could not open gemBS config file {} for writing: {}",
        ...
        let now = Instant::now();
        rmp_serde::encode::write(&mut writer, &self.var).map_err(|e| {
            format!(
                "Error: failed to write RMP config file {}: {}",
                config_file.to_string_lossy(),
                e
            )
        })?;
        debug!(
            "Config file written out to {} in {}ms",
            config_file.display(),
    ...
    pub fn read_config(&mut self) -> Result<(), String> {
        self.check_signal()?;
        let config_file = &self.fs.as_ref().unwrap().config_file;
        let lock = timed_wait_for_lock(self.get_signal_clone(), config_file)
            .map_err(|e| format!("Error: Could not obtain lock on JSON config file: {}", e))?;
        let reader = lock.reader().map_err(|e| {
            format!(
        ...
        let now = Instant::now();
        self.var = rmp_serde::decode::from_read(reader).map_err(|e| {
            format!(
                "Error: failed to read MP config file {}: {}",
                config_file.to_string_lossy(),
                e
            )
        })?;
        debug!(
            "config file read from {} in {}ms",
            config_file.display(),
            now.elapsed().as_millis()
        );
        self.check_signal()?;
    ...
    pub fn setup_fs(&mut self, initial: bool) -> Result<(), String> {
        ...
        let cdir = ".gemBS";
        let config_file =
            if let Some(DataValue::String(x)) = self.get_config(Section::Default, "config_file") {
                PathBuf::from(x)
            } else {
        ...
        let gem_bs_root =
            if let Some(DataValue::String(x)) = self.get_config(Section::Default, "gembs_root") {
                PathBuf::from(x)
            } else if let Ok(x) = env::var("GEMBS_ROOT") {
                PathBuf::from(x)
            } else if let Some(x) = compile_root {
                PathBuf::from(x)
            } else {
        ...
        if initial {
            if config_dir.exists() {
                if !config_dir.is_dir() {
                    return Err(format!(
                        "Could not create config directory {:?} as file exists with same name",
                        config_dir
                    ));
                }
            ...
            if !config_dir.is_dir() {
                return Err(format!(
                    "Config directory {} does not exist (or is not accessible)",
                    config_dir.display()
                ));
            }
            if !config_file.exists() {
                return Err(format!(
                    "Config file {} does not exist (or is not accessible)",
                    config_file.display()
                ));
            }
        }
        ...
        let mode = config_dir
            ...
            .map_err(|e| {
                format!(
                    "Could not get metadata for config dir {}: {}",
                    config_dir.display(),
                    e
                )
            })?
            .permissions()
            .mode();
        if (mode & 0o1700) != 0o0700 {
            return Err(format!(
                "Incorrect permissions for config_dir {}",
                config_dir.display()
            ));
        }

        debug!("gem_bs_root set to {}", gem_bs_root.display());
        self.fs = Some(GemBSFiles {
    ...
    pub fn get_task_file_path(&self) -> PathBuf {
        [
            &self.fs.as_ref().unwrap().config_dir,
            Path::new("gemBS_tasks.json"),
        ]
        .iter()
        .collect()
    }
    pub fn get_config_script_path(&self) -> PathBuf {
        let root = &self.fs.as_ref().unwrap().gem_bs_root;
        [root, Path::new("etc"), Path::new("config_scripts")]
            .iter()
            .collect()
    }
    ...
    pub fn setup_assets_and_tasks(&mut self, lock: &FileLock) -> Result<(), String> {
        ...
        for asset in self.get_assets().iter() {
            ...
            for j in asset.parents() {
                if *j > i {
                    error!(
                        "Parent asset {}:{} larger number than child {}:{}",
                        *j,
                        self.get_asset(*j).unwrap().id(),
                        i,
                        self.get_asset(i).unwrap().id()
                    );
                }
            }
        }
    ...
    pub fn rescan_assets_and_tasks(&mut self, lock: &FileLock) -> Result<(), String> {
        ...
        let running_ids = running.iter().map(|x| self.tasks.find_task(x.id())).fold(
            HashSet::new(),
            |mut h, x| {
                if let Some(t) = x {
                    self.tasks[t].outputs().for_each(|y| {
                        h.insert(*y);
                    })
                }
                h
    ...
    fn calc_task_statuses(&mut self, running: &[RunningTask]) {
        let hset: HashSet<&str> = running.iter().fold(HashSet::new(), |mut hr, x| {
            hr.insert(x.id());
            hr
        });
        let svec: Vec<(usize, TaskStatus)> = self
            .tasks
            .iter()
            .map(|x| {
                let mut s = self.task_status(x);
                if hset.contains(x.id()) {
                    if s != TaskStatus::Ready && s != TaskStatus::Complete {
                        warn!(
                            "Task {} in running queue in {:?} instead of Ready or Complete state",
                            x.id(),
                            s
                        )
                    }
        ...
        svec.iter().for_each(|(ix, s)| {
            if Some(*s) != self.tasks[*ix].status() {
                ...
                trace!(
                    "calc_task_statuses(): switching task {} status from {:?} to {:?}",
                    task.id(),
                    task.status(),
                    *s
                );
    ...
    pub fn set_ignore_times(&mut self, x: bool) {
        self.ignore_times = x;
    }
    pub fn ignore_times(&self) -> bool {
        self.ignore_times
    }
    pub fn set_ignore_status(&mut self, x: bool) {
        self.ignore_status = x;
    }
    pub fn ignore_status(&self) -> bool {
        self.ignore_status
    }
    pub fn set_keep_logs(&mut self, x: bool) {
        self.keep_logs = x;
    }
    pub fn keep_logs(&self) -> bool {
        self.keep_logs
    }
    pub fn set_all(&mut self, x: bool) {
        self.all = x;
    }
    pub fn all(&self) -> bool {
        self.all
    }
    pub fn set_verbose(&mut self, verbose: LogLevel) {
        self.verbose = verbose;
    }
    pub fn verbose(&self) -> LogLevel {
        self.verbose
    }
    pub fn set_dry_run(&mut self, x: bool) {
        self.dry_run = x;
    }
    pub fn dry_run(&self) -> bool {
        self.dry_run
    }
    pub fn set_slurm(&mut self, x: bool) {
        self.slurm = x;
    }
    pub fn set_slurm_script(&mut self, x: &str) {
        self.slurm_script = Some(x.to_owned());
    }
    ...
    pub fn execute_flag(&self) -> bool {
        !(self.dry_run || self.slurm || self.json_out.is_some() || self.slurm_script.is_some())
    }
    pub fn get_required_tasks_from_asset_list(
        &self,
        assets: &[usize],
        com_list: &[Command],
    ) -> Vec<usize> {
        let com_set = com_list.iter().fold(HashSet::new(), |mut hs, x| {
            hs.insert(*x);
            hs
        });
        fn check_reqd(
            i: usize,
            reqd: &mut HashSet<usize>,
            tlist: &mut Vec<usize>,
            rf: &TaskList,
            arf: &AssetList,
            com_set: &HashSet<Command>,
            ignore: bool,
        ) {
            if !reqd.contains(&i) {
                reqd.insert(i);
                ...
                if (ignore || st != TaskStatus::Complete) && com_set.contains(&rf[i].command()) {
                    tlist.push(i);
                }
            }
        }
    ...
    pub fn get_mapping_json_files_for_barcode(&self, barcode: &str) -> Vec<usize> {
        ...
        if let Some(t) = self
            .tasks
            .find_task(format!("single_map_{}", barcode).as_str())
        {
            for i in self.tasks[t].outputs() {
                let asset = self.get_asset(*i).expect("Couldn't get asset");
                if asset.id().ends_with(".json") {
                    json_files.push(asset.idx())
                }
            }
        } else if let Some(t) = self
            .tasks
            .find_task(format!("merge-bam_{}", barcode).as_str())
        {
            ...
            panic!("Couldn't find map tasks for barcode");
        }
        json_files
    }
}
    ...
    if path.is_dir() {
        ...
        if tpath.is_dir() {
            for app in apps.iter() {
                tpath.push(app);
                if tpath.is_file() {
                    trace!("Checking for {} in {:?}: OK", app, tpath);
                } else {
        ...
        if tpath.is_dir() {
            ...
            trace!(
                "Checking for config script directory in {:?}: Not found",
                tpath
            );
            false
        }
