...
use utils::compress;
use crate::config::*;
use crate::htslib::{SamHeader, Faidx};
...
impl CtgInfo {
	fn new() -> Self {CtgInfo { ref_id: None, in_header: false}}
...
fn read_omit_ctg_list(chash: &ConfHash, sam_hdr: &SamHeader) -> io::Result<HashSet<usize>> {
			...
			match rdr.read_line(&mut line) {
				...
				Ok(_) => {
					...
					if let Some(ctg) = iter.next() {
						let ctg = ctg.trim();
						if !ctg.is_empty() {
							if let Some(x) = sam_hdr.name2tid(ctg) { 
...
fn setup_ref_ids(ctgs: &mut Vec<CtgInfo>, sam_hdr: &SamHeader, ref_idx: &Faidx) {
	for i in 0..ref_idx.nseq() {
		let name = ref_idx.iseq(i);
		if let Some(tid) = sam_hdr.name2tid(name) {
...
fn setup_regions_with_contig_bed(fname: &str, filter: bool, ctgs: &mut Vec<CtgInfo>, sam_hdr: &SamHeader, ref_idx: &Faidx, omit_ctgs: &HashSet<usize>) -> io::Result<Vec<CtgRegion>> {
					...
					if let Some(tid) = sam_hdr.name2tid(name) {
						if !omit_ctgs.contains(&tid) {
							...
							if let Some(len) = ref_idx.seq_len(name) {
								if sam_hdr.tid2len(tid) != len { return Err(new_err(format!("Mismatch in sequence lengths between reference and SAM header for contig {}: {}, {}", name, len, sam_hdr.tid2len(tid)))); }
								let len = sam_hdr.tid2len(tid);
								...
								if let (Ok(x), Ok(y)) = (start, stop) {
									...
									ctg_regions.push(CtgRegion::new(tid, x + 1, y));
	...
	ctgs.iter().enumerate().filter(|(_,c)| c.in_header).for_each(|(i,_)| trace!("Contig {} will be output in VCF header", sam_hdr.tid2name(i)));
	debug!("Done");
	Ok(ctg_regions)	
}

fn setup_regions_from_sam_header(ctgs: &mut Vec<CtgInfo>, sam_hdr: &SamHeader, ref_idx: &Faidx, omit_ctgs: &HashSet<usize>) -> io::Result<Vec<CtgRegion>> {
	let mut ctg_regions = Vec::new();
	for (i, c) in ctgs.iter_mut().enumerate() {
		if !omit_ctgs.contains(&i) {
			let name = sam_hdr.tid2name(i);
			if c.ref_id.is_some() {
				if let Some(l) = ref_idx.seq_len(name) {
					if sam_hdr.tid2len(i) != l { return Err(new_err(format!("Mismatch in sequence lengths between reference and SAM header for contig {}: {}, {}", name, l, sam_hdr.tid2len(i)))); }
					c.in_header = true;
					trace!("Contig {} will be output in VCF header", name);
					ctg_regions.push(CtgRegion::new(i, 1, l));
...
pub fn setup_contigs(chash: &ConfHash, sam_hdr: &SamHeader, ref_idx: &Faidx) -> io::Result<(Vec<CtgInfo>, Vec<CtgRegion>)> {
	let omit_ctgs = read_omit_ctg_list(chash, sam_hdr)?;
	let mut ctgs = init_sam_contigs(sam_hdr.nref());
	setup_ref_ids(&mut ctgs, sam_hdr, ref_idx);
	let ctg_regions = if let Some(fname) = chash.get_str("contig_bed") {
		let filter = chash.get_bool("filter_contigs");
		setup_regions_with_contig_bed(fname, filter, &mut ctgs, sam_hdr, ref_idx, &omit_ctgs)?
	} else {
		setup_regions_from_sam_header(&mut ctgs, sam_hdr, ref_idx, &omit_ctgs)?
	};
