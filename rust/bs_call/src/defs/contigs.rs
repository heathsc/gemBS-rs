...
use crate::config::*;
use crate::htslib::{Faidx, SamFile, VcfHeader, BCF_DT_CTG};
use utils::compress;
...
impl CtgInfo {
    ...
    pub fn vcf_id(&self) -> Option<usize> {
        self.vcf_id
    }
    pub fn ref_id(&self) -> Option<usize> {
        self.ref_id
    }
...
impl CtgRegion {
    fn new(sam_tid: usize, start: usize, stop: usize) -> Self {
        if stop < start {
            panic!("CtgRegion::new() stop < start")
        }
        CtgRegion {
            sam_tid,
            start,
                    ...
                    if let Some(ctg) = iter.next() {
                        ...
                        if !ctg.is_empty() {
                            if let Some(x) = sam_file.name2tid(ctg) {
                                ...
                                warn!("Contig {} in contig {} file {} not present in SAM header (ignored)", ctg, key, fname)
                            }
                        }
                    }
                    line.clear();
...
fn init_sam_contigs(sam_file: &SamFile) -> Vec<CtgInfo> {
    (0..sam_file.nref()).fold(Vec::new(), |mut v, i| {
        v.push(CtgInfo::new(sam_file.tid2name(i)));
        v
    })
}

fn setup_ref_ids(ctgs: &mut [CtgInfo], sam_file: &SamFile, ref_idx: &Faidx) {
    for i in 0..ref_idx.nseq() {
        let name = ref_idx.iseq(i);
        if let Some(tid) = sam_file.name2tid(name) {
            ctgs[tid].ref_id = Some(i);
            trace!(
                "Matched Reference contig {} ({}) to SAM header sequence {}",
                i,
                name,
                tid
            );
...
fn setup_regions_with_contig_bed(
    fname: &str,
    filter: bool,
    ctgs: &mut [CtgInfo],
    sam_file: &SamFile,
    ref_idx: &Faidx,
    omit_ctgs: &HashSet<usize>,
    include_ctgs: &HashSet<usize>,
) -> io::Result<Vec<CtgRegion>> {
    ...
    if !filter {
        if include_ctgs.is_empty() {
            ctgs.iter_mut()
                .enumerate()
                .for_each(|(i, c)| c.in_header = !omit_ctgs.contains(&i))
        } else {
            ctgs.iter_mut().enumerate().for_each(|(i, c)| {
                c.in_header = include_ctgs.contains(&i) && !omit_ctgs.contains(&i)
            })
        }
    }
    let mut rdr = compress::open_bufreader(fname)?;
                ...
                if v.len() >= 3 {
                    ...
                    if let Some(tid) = sam_file.name2tid(name) {
                        if !omit_ctgs.contains(&tid) {
                            if ctgs[tid].ref_id.is_none() {
                                return Err(new_err(format!(
                                    "Contig {} missing in reference FASTA",
                                    name
                                )));
                            }
                            if let Some(len) = ref_idx.seq_len(name) {
                                if sam_file.tid2len(tid) != len {
                                    return Err(new_err(format!("Mismatch in sequence lengths between reference and SAM header for contig {}: {}, {}", name, len, sam_file.tid2len(tid))));
                                }
                                ...
                                if let (Ok(x), Ok(mut y)) = (start, stop) {
                                    ...
                                    if x >= y {
                                        return Err(new_err(format!("Error parsing region list file {} at line {}: coordinates out of range", fname, l)));
                                    }
                                    ctgs[tid].in_header = true;
                                    ctg_regions.push(CtgRegion::new(tid, x, y - 1));
                                    trace!("Added region {}:{}-{}", name, x + 1, y);
                                } else {
                                    return Err(new_err(format!("Error parsing region list file {} at line {}: coordinates invalid", fname, l)));
                                }
                            } else {
                                panic!(
                                    "Missing sequence length from reference index for contig {}",
                                    name
                                );
                            }
                        }
                        ...
                        return Err(new_err(format!(
                            "Contig {} in region list file {} not present in SAM header",
                            name, fname
                        )));
                    }
                } else if !(v.len() == 1 && v[0].trim().is_empty()) {
                    return Err(new_err(format!(
                        "Error parsing region list file {} at line {}",
                        fname, l
                    )));
                }
            }
    ...
    ctgs.iter()
        .enumerate()
        .filter(|(_, c)| c.in_header)
        .for_each(|(i, _)| {
            trace!(
                "Contig {} will be output in VCF header",
                sam_file.tid2name(i)
            )
        });
    debug!("Done");
...
fn setup_regions_from_sam_header(
    ctgs: &mut [CtgInfo],
    sam_file: &SamFile,
    ref_idx: &Faidx,
    omit_ctgs: &HashSet<usize>,
        ...
        if !omit_ctgs.contains(&i) && (include_ctgs.is_empty() || include_ctgs.contains(&i)) {
            ...
            if c.ref_id.is_some() {
                if let Some(l) = ref_idx.seq_len(name) {
                    if sam_file.tid2len(i) != l {
                        return Err(new_err(format!("Mismatch in sequence lengths between reference and SAM header for contig {}: {}, {}", name, l, sam_file.tid2len(i))));
                    }
                    c.in_header = true;
...
pub fn setup_contigs(
    chash: &ConfHash,
    sam_file: &SamFile,
    ref_idx: &Faidx,
) -> io::Result<(Vec<CtgInfo>, Vec<CtgRegion>)> {
...
pub fn set_contig_vcf_ids(vcf_hdr: &VcfHeader, ctgs: &mut [CtgInfo], sam_file: &SamFile) {
    ...
    for (tid, vid) in vcf_ids.iter().enumerate() {
        if ctgs[tid].in_header {
            if vid.is_none() {
                panic!("Contig not found in BCF/VCF header");
            }
            ctgs[tid].vcf_id = *vid;
        }
    }
