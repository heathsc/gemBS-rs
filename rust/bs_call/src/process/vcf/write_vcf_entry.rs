...
use crate::htslib::*;
use crate::process::call_genotypes::{CallBlock, CallEntry, GenotypeCall};
use crate::rusage::*;
...
pub const FLT_NAMES: [&str; 16] = [
    "PASS", "mac1", "fail", "GT", "FT", "DP", "MQ", "GQ", "QD", "GL", "MC8", "AMQ", "CS", "CG",
    "CX", "FS",
];
...
fn get_filter_ids(hdr: &VcfHeader) -> Vec<u8> {
    ...
    for flt in FLT_NAMES.iter() {
        v.push(
            hdr.id2int(BCF_DT_ID as usize, flt)
                .expect("mac1 filter not found in VCF header") as u8,
        )
    }
    v
}
...
const CPG_ST_CTS: [Option<(usize, usize)>; 10] = [
    None,
    Some((5, 7)),
    Some((6, 4)),
    None,
...
fn ln_prob_2_phred(p: f64) -> c_int {
    ...
    if z >= 1.0 {
        ...
        cmp::min((-10.0 * (1.0 - z).ln() / LN_10) as c_int, 255)
    }
}
...
fn enc_vfloat(v: &mut Vec<u8>, s: &[f32]) {
    ...
    s.iter()
        .copied()
        .for_each(|x| v.extend_from_slice(&x.to_le_bytes()));
}

fn enc_vint(v: &mut Vec<u8>, s: &[c_int]) {
    let (min, max): (c_int, c_int) = s[1..].iter().copied().fold((s[0], s[0]), |(mn, mx), x| {
        if x < mn {
            (x, mx)
        } else if x > mx {
            (mn, x)
...
fn enc_vint_(v: &mut Vec<u8>, s: &[c_int], min: c_int, max: c_int) {
    if max <= BCF_MAX_BT_INT8 && min >= BCF_MIN_BT_INT8 {
        ...
        s.iter()
            .copied()
            .for_each(|x| v.extend_from_slice(&(x as u16).to_le_bytes()));
    } else {
        enc_size(v, s.len() as c_int, BCF_BT_INT32);
        s.iter()
            .copied()
            .for_each(|x| v.extend_from_slice(&(x as u32).to_le_bytes()));
    }
}
...
fn write_format_columns(
    ...
    enc_vfloat(v, &gl);
    // MC8
    let (max, nz) = call.counts.iter().copied().fold((0, 0), |(m, n), x| {
        if x > m {
            (x, n + 1)
        } else if x > 0 {
            (m, n + 1)
    ...
    if nz > 0 {
        ...
        call.qual.iter().copied().for_each(|q| {
            if q > 0 {
                v.push(q as u8)
            }
        });
...
impl WriteState {
    fn new_block(mut call_block: CallBlock, bs_cfg: &BsCallConfig) -> Self {
        ...
        for _ in 0..3 {
            v.push_back(CallEntry::Starting(0))
        }
        for c in call_block.prec_ref_bases.iter() {
            v.push_back(CallEntry::Starting(*c))
        }
    ...
    fn finish_block(
        mut self,
        vcf_output: &mut VcfFile,
        filter_ids: &[u8],
        vcf_stats_tx: &mpsc::SyncSender<Option<Vec<CallStats>>>,
    ) -> io::Result<()> {
        for _ in 0..2 {
    ...
    fn write_entry(&mut self, vcf_output: &mut VcfFile, filter_ids: &[u8]) -> io::Result<()> {
        match &self.call_buf[2] {
            CallEntry::Call(call) => {
                ...
                let (rs, rs_reqd) = match if let Some(ctg) = &self.dbsnp_contig {
                    ctg.lookup_rs(self.curr_x)
                } else {
                    None
                } {
                    Some((s, fg)) => (Some(s), fg),
                ...
                let skip = !self.all_positions
                    && !rs_reqd
                    && !GT_FLAG[call.max_gt as usize][call.ref_base as usize];
                ...
                let called_context: Vec<u8> = called_gt
                    .iter()
                    .copied()
                    .map(|g| IUPAC.as_bytes()[g])
                    .collect();

                let cpg_status = {
                    cmp::max(
                        CPG_STATE[called_gt[1]][called_gt[2]],
                        CPG_STATE[called_gt[2]][called_gt[3]],
                    ) | if (ref_context[2] == b'C' && ref_context[3] == b'G')
                        || (ref_context[1] == b'C' && ref_context[2] == b'G')
                    {
                        CPG_STATUS_REF_CPG
                    } else {
                        0
                ...
                let flags = if skip { CALL_STATS_SKIP } else { 0 }
                    | if rs_found { CALL_STATS_RS_FOUND } else { 0 };
                let meth_cts = CPG_ST_CTS[call.max_gt as usize]
                    .map(|(x, y)| (call.counts[x] as usize, call.counts[y] as usize));
                let mut call_stats = CallStats {
                    sam_tid: self.sam_tid,
                    phred,
                    fs,
                    dp1,
                    d_inf,
...
pub fn write_vcf_entry(
    bs_cfg: Arc<BsCallConfig>,
    rx: mpsc::Receiver<WriteVcfJob>,
    ...
    let vcf_stats_handle =
        thread::spawn(move || collect_vcf_stats(Arc::clone(&bs_cfg), vcf_stats_rx, stat_tx));
        ...
        match rx.recv() {
            Ok(WriteVcfJob::Quit) => {
                if let Some(ws) = write_state.take()
                    && let Err(e) = ws.finish_block(&mut vcf_output, &filter_ids, &vcf_stats_tx)
                {
            ...
            Ok(WriteVcfJob::GenotypeCall(call_vec)) => match write_state.as_mut() {
                Some(ws) => {
    ...
    if vcf_stats_tx.send(None).is_err() {
        warn!("Error trying to send QUIT signal to vcf_stats thread")
    }
    if vcf_stats_handle.join().is_err() {
        warn!("Error waiting for vcf_stats thread to finish")
    }
    if let Ok(ru_thread) = Rusage::get(RusageWho::RusageThread) {
        info!(
            "write_vcf_thread shutting down: user {} sys {}",
            ru_thread.utime(),
            ru_thread.stime()
        );
    }
