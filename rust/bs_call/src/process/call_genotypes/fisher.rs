...
impl FisherTest {
    ...
    pub fn calc_fs_stat(&self, mx: usize, cts: &[u32]) -> f64 {
        if GT_HET[mx] {
            let get_cts = |x: &[usize], y: &[usize]| {
                [
                    x.iter().fold(0, |s, x| s + cts[*x]),
                    y.iter().fold(0, |s, x| s + cts[*x]),
                    x.iter().fold(0, |s, x| s + cts[*x + 8]),
                    y.iter().fold(0, |s, x| s + cts[*x + 8]),
                ]
            };
    ...
    pub fn fisher(&self, ftab: &[u32; 4]) -> f64 {
        ...
        if n < 1.0 {
            ...
            if delta > 0.0 {
                ...
                for i in 0..min {
                    like *= (((c[1] - i) * (c[2] - i)) as f64)
                        / (((c[0] + i + 1) * (c[3] + i + 1)) as f64);
                    prob += like;
                }
                ...
                if adjust <= min {
                    ...
                    for i in 0..min - adjust {
                        like *= (((c[0] - i) * (c[3] - i)) as f64)
                            / (((c[1] + i + 1) * (c[2] + i + 1)) as f64);
                        prob += like;
                    }
                }
            } else {
