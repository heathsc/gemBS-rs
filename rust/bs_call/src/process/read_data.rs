...
use super::pileup;
use super::records::{Map, ReadEnd};
use crate::config::{BsCallConfig, BsCallFiles};
use crate::htslib::*;
use crate::stats::{FSReadLevelType, FSType, StatJob};
    ...
    fn update(&mut self, map: &Map, idx: usize) -> StateChange {
        if let Some(mut state) = self.0.take() {
            ...
            if mp.tid != state.tid {
                ...
                state.tid = mp.tid;
                state_cp(&mut state);
            } else if start_x > state.end_x + 2 {
                // Want at least 2 bases free between blocks to avoid issues with call context in VCF output
                change = StateChange::NewBlock((state.start_x, state.end_x));
                state_cp(&mut state);
            } else {
                if start_x < state.curr_x {
                    panic!("BAM is not sorted!")
                }
                if start_x > state.curr_x {
...
fn send_pileup_job(
    ...
    y: u32,
    tid: u32,
    pileup_tx: &mpsc::SyncSender<Option<pileup::PileupRegion>>,
) -> io::Result<()> {
    ...
    match pileup_tx.send(Some(preg)) {
        Err(e) => {
            ...
            Err(hts_err(format!(
                "Error sending region to pileup thread: {}",
                e
            )))
        }
        Ok(_) => Ok(()),
...
fn count_passed_reads(reads: &[Option<ReadEnd>], fs_stats: &mut FSType) {
    for read in reads.iter().flatten() {
        if read.is_primary() {
            fs_stats.add_read_level_count(FSReadLevelType::Passed, read.seq_qual.len());
        }
    }
}
...
pub fn read_data(
    ...
    let pileup_handle =
        thread::spawn(move || pileup::make_pileup(Arc::clone(&bs_cfg), pileup_rx, bs_files, st_tx));
        ...
        if let Some(mut read) = read_end {
            ...
            match change {
                ...
                StateChange::NewContig((tid, x, y)) => {
                    ...
                    trace!(
                        "Ending contig with block ({}:{}-{} len = {}, {} maps)",
                        cname,
                        x,
                        y,
                        y - x + 1,
            ...
            let insert = if let Some(state) = state_hash.get(id) {
                match state {
                    ReadState::Duplicate => {
                        if read.is_primary() {
                            fs_stats.add_read_level_count(
                                FSReadLevelType::Duplicate,
                                brec.l_qseq() as usize,
                            );
                        }
                        false
                    }
            ...
            // Only collect stats on primary reads unless they are flagged for being secondary or supplementary
            if match read_flag {
                FSReadLevelType::SupplementaryAlignment | FSReadLevelType::SecondaryAlignment => {
                    true
                }
    ...
    if pileup_tx.send(None).is_err() {
        warn!("Error trying to send QUIT signal to pileup thread")
    } else {
        for (flag, ct) in fs_stats.read_level().iter() {
            let _ = stat_tx.send(StatJob::AddFSReadLevelCounts(*flag, *ct));
        }
        if pileup_handle.join().is_err() {
            warn!("Error waiting for pileup thread to finish")
        }
    }
    Ok(())
}
