...
impl PartialEq for Map {
    fn eq(&self, other: &Self) -> bool {
        self.map_pos == other.map_pos && ((self.flags ^ other.flags) & MFLAG_DUPLICATE_MASK) == 0
    }
}

const BSSTRAND_MAP: [BSStrand; 4] = [
    BSStrand::Unknown,
    BSStrand::StrandC2T,
    BSStrand::StrandG2A,
    BSStrand::Unconverted,
];
...
impl Map {
    pub fn bs_strand(&self) -> BSStrand {
        BSSTRAND_MAP[(self.flags & MFLAG_BSSTRAND_MASK) as usize]
    }
    pub fn is_last(&self) -> bool {
        (self.flags & MFLAG_LAST) != 0
    }
    pub fn is_reverse(&self) -> bool {
        (self.flags & MFLAG_REVERSE) != 0
    }
    pub fn start(&self) -> u32 {
        self.map_pos.pos
    }
    pub fn end(&self) -> u32 {
        self.map_pos.pos + self.cigar.rlen() - 1
    }
...
fn maps_from_bam_rec(
    ...
    if tst_flag(BAM_FUNMAP) {
        return Err("Read unmapped".to_string());
    }
    let (bs_strand, sa_tag) = get_bs_strand(brec);
    let mut flags = bs_strand.get_num();
    if tst_flag(BAM_FDUP) {
        flags |= MFLAG_DUPLICATE
    };
    if tst_flag(BAM_FREVERSE) {
        flags |= MFLAG_REVERSE
    };
    if tst_flag(BAM_FREAD1) {
        flags |= MFLAG_READ1
    };
    if tst_flag(BAM_FREAD2) {
        flags |= MFLAG_READ2
    };
    if tst_flag(BAM_FSUPPLEMENTARY) {
        flags |= MFLAG_SPLIT
    };
    if tst_flag(BAM_FSECONDARY | BAM_FSUPPLEMENTARY) {
        flags |= MFLAG_NON_PRIMARY
    }
    let mut maps = Vec::with_capacity(1);
    let tid = if let Some(i) = brec.tid() {
        i as u32
    } else {
        return Err("Invalid contig ID".to_string());
    };
    let pos = if let Some(i) = brec.pos() {
        i as u32
    } else {
        return Err("Invalid map position".to_string());
    };
    ...
    if maps.len() > 1 {
        if maps.len() > 256 {
            panic!("Too many splits in read id {} (max = 256)", brec.qname())
        }
        ...
        map_idx.sort_by_cached_key(|ix| {
            ...
            let cig_elem = if (maps[*ix].flags & MFLAG_REVERSE) == 0 {
                &cig[0]
            } else {
                cig.last().unwrap()
            };
        ...
        for idx in map_idx.iter() {
            maps[*idx].flags |= if maps[*idx].bs_strand() == BSStrand::Unknown {
                bs_strand.get_num() | MFLAG_SPLIT
            } else {
                MFLAG_SPLIT
            };
...
fn map_from_sa_tag(sam_hdr: &SamHeader, tag: &str) -> Result<Map, String> {
    ...
    if !v.is_empty() {
        if v.len() != 6 {
            return Err(format!(
                "Wrong number of fields in SA tag (expected 6, found {})",
                v.len()
            ));
        }
        let tid = sam_hdr
            .name2tid(v[0])
            .ok_or_else(|| format!("Unknown contig {} in SA tag {}", v[0], tag))?
            as u32;
        let pos = <u32>::from_str(v[1])
            .map_err(|e| format!("Couldn't parse position {} in SA tag {}: {}", v[1], tag, e))?;
        let flags = match v[2] {
            "+" => MFLAG_SPLIT,
        ...
        let mapq = if let Ok(x) = <u8>::from_str(v[4]) {
            x
        } else {
            panic!("Couldn't parse mapq {} in SA tag {}", v[4], tag)
        };
        Ok(Map {
            map_pos: MapPos { pos, tid },
            flags,
            cigar,
            mapq,
            split_id: 0,
...
impl ReadEnd {
    pub fn from_bam_rec(
        ...
        let tst_flag = |x| (flag & x) != 0;
        let mut set_read_flag = |x| {
            if read_flag == ReadFlag::Passed {
                read_flag = x
            }
        };
        // Filter out the simple cases
        if tst_flag(BAM_FUNMAP) {
            return (None, ReadFlag::Unmapped);
        };
        if tst_flag(BAM_FQCFAIL) {
            return (None, ReadFlag::QCFlags);
        };
        if tst_flag(BAM_FSECONDARY) {
            return (None, ReadFlag::SecondaryAlignment);
        };
        if !ignore_duplicates && tst_flag(BAM_FDUP) {
            if keep_duplicates {
                set_read_flag(ReadFlag::Duplicate)
            } else {
                return (None, ReadFlag::Duplicate);
            }
        }
        if (brec.qual() as usize) < mapq_threshold {
            return (None, ReadFlag::LowMAPQ);
        };
        if tst_flag(BAM_FSUPPLEMENTARY) {
            if keep_supplementary {
                set_read_flag(ReadFlag::SupplementaryAlignment)
            } else {
                return (None, ReadFlag::SupplementaryAlignment);
            }
        }
        let tid = if let Some(x) = brec.tid() {
            x
        } else {
            return (None, ReadFlag::NoPosition);
        };
        if brec.pos().is_none() {
            return (None, ReadFlag::NoPosition);
        };
        let mate_pos = if tst_flag(BAM_FPAIRED) {
            if tst_flag(BAM_FMUNMAP) {
                if keep_unmatched {
                    set_read_flag(ReadFlag::MateUnmapped)
                } else {
                    return (None, ReadFlag::MateUnmapped);
                }
                None
            } else {
                let mtid = if let Some(x) = brec.mtid() {
                    x
                } else {
                    return (None, ReadFlag::NoMatePosition);
                };
                let mpos = if let Some(x) = brec.mpos() {
                    x
                } else {
                    return (None, ReadFlag::NoMatePosition);
                };
                if mtid != tid {
                    if keep_unmatched {
                        set_read_flag(ReadFlag::MisMatchContig)
                    } else {
                        return (None, ReadFlag::MisMatchContig);
                    }
                }
                if brec.template_len().abs() > max_template_length as isize {
                    if keep_unmatched {
                        set_read_flag(ReadFlag::LargeInsertSize)
                    } else {
                        return (None, ReadFlag::LargeInsertSize);
                    }
                }
                Some(MapPos {
                    tid: mtid as u32,
        ...
        let maps = maps_from_bam_rec(sam_hdr, brec, keep_supplementary).unwrap_or_else(|e| {
            panic!(
                "Couldn't get map record from BAM read (id: {}: {}",
                brec.qname(),
                e
            )
        });
        let seq_qual = brec
            .get_seq_qual()
            .unwrap_or_else(|e| panic!("Error in BAM: (id: {} - {})", brec.qname(), e));
        (
            Some(ReadEnd {
                seq_qual,
                maps,
    ...
    fn maps_eq(&self, maps: &[Map]) -> bool {
        if self.maps.len() == maps.len() {
            for (m1, m2) in self.maps.iter().zip(maps.iter()) {
                if m1 != m2 {
                    return false;
                }
            }
    ...
    pub fn check_dup(&self, read_list: &[Option<Self>]) -> bool {
        for r in read_list.iter().flatten() {
            if self.maps_eq(&r.maps)
                && match (r.mate_pos, self.mate_pos) {
                    (Some(x), Some(y)) => x == y,
                    _ => true,
                }
            {
    ...
    pub fn check_pair(&mut self, read: &mut Self, conf_hash: &ConfHash) -> (bool, ReadFlag) {
        ...
        if map1.map_pos.tid != map2.map_pos.tid {
            return (!keep_unmatched, ReadFlag::MisMatchContig);
        }
        // Check if maps are on different strands
        if ((map1.flags ^ map2.flags) & MFLAG_REVERSE) == 0 {
            return (!keep_unmatched, ReadFlag::BadOrientation);
        }
        ...
        if (map1.flags & MFLAG_REVERSE) == 0 {
            ...
            let end2 = map2.end();
            if end2 - map1.start() >= max_template_length {
                return (!keep_unmatched, ReadFlag::LargeInsertSize);
            }
            // Check for overlap
            let end1 = map1.end();
            if end1 >= map2.start() {
    ...
    pub fn read_one(&self) -> bool {
        (self.maps[0].flags & (MFLAG_READ1 | MFLAG_READ2)) == MFLAG_READ1
    }
    pub fn read_two(&self) -> bool {
        (self.maps[0].flags & (MFLAG_READ1 | MFLAG_READ2)) == MFLAG_READ2
    }
    pub fn is_primary(&self) -> bool {
        (self.maps[0].flags & MFLAG_NON_PRIMARY) == 0
    }
}
