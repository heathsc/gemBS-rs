    ...
    pub fn unload_ctg<S: AsRef<str>>(&mut self, name: S) {
        ...
        if let Some(ctg) = self.index.dbsnp.get_mut(name) {
            if ctg.ctg.bins.is_some() {
                ...
                warn!("Couldn't unload dbSNP data for {}: no data present", name)
            }
        }
    }
...
impl DBSnpContig {
    pub fn lookup_rs(&self, x: usize) -> Option<(String, bool)> {
        if let Some(bins) = &self.bins {
            ...
            if bn >= self.min_bin
                && bn <= self.max_bin
                && let Some(bin) = &bins[bn - self.min_bin]
            {
                return bin.lookup_rs((x + 1) & 255);
            }
        }
        None
    }
    ...
    pub fn new<S: AsRef<str>>(name: S) -> io::Result<Self> {
        ...
        if td[0] != 0xd7278434 {
            return Err(new_err(format!(
                "Invalid format: bad magic number {:x}",
                td[0]
            )));
        }
        ...
        if vs[0] != 2 {
            return Err(new_err("Invalid version number".to_string()));
        }
        let mut td1 = [0u64; 3];
        ...
        if td[0] != 0xd7278434 {
            return Err(new_err(
                "Invalid format: bad second magic number".to_string(),
            ));
        }
        ...
        if sz < 4 {
            return Err(new_err("Invalid format: short header".to_string()));
        }
        ...
        if !p.is_empty() {
            Err(new_err(
                "Error with dbSNP index header - excess data".to_string(),
            ))
        } else {
            trace!("Contigs read in OK");
            info!(
                "Read dbSNP header from {} with data on {} contigs",
                filename, n_ctgs
            );
...
impl DBSnpBin {
    fn lookup_rs(&self, ix: usize) -> Option<(String, bool)> {
        ...
        if (self.mask[k] & mk) != 0 {
            ...
            let start_x: usize = self.name_len[0..n_prev_entries]
                .iter()
                .map(|x| *x as usize)
                .sum();
            ...
            if (start_x & 1) != 0 {
                rs.push(DTAB[(it.next().expect("Short name buf") & 0xf) as usize])
            }
            let select = loop {
...
impl DBSnpCtg {
    ...
    fn load_data(&mut self, mut file: &mut BufReader<File>, bufsize: usize) -> io::Result<()> {
        ...
        loop {
            ...
            if first_bin < self.min_bin() + bins.len() {
                return Err(new_err("Error: index data corrupt".to_string()));
            }
        ...
        if bins.len() != self.max_bin() + 1 - self.min_bin() {
            Err(new_err(format!(
                "Wrong number of bins read in.  Expected {}, Found {}",
                self.max_bin() + 1 - self.min_bin(),
                bins.len()
            )))
        } else {
            let n_snps = bins.iter().fold(0, |s, b| {
                if let Some(bin) = b {
                    s + bin.name_len.len()
                } else {
                    s
...
fn load_bins(mut buf: &[u8], gap: usize) -> io::Result<Vec<Option<DBSnpBin>>> {
    ...
    loop {
        ...
        bins.push(Some(DBSnpBin {
            mask,
            name_buf: name_buf.into_boxed_slice(),
            name_len: name_len.into_boxed_slice(),
        }));
    }
    Ok(bins)
}
...
fn get_ctg_header(buf: &[u8]) -> io::Result<DBSnpCtg> {
    if buf.len() >= 16 {
        ...
        Err(new_err(
            "Bad format: Failed to read in contig header".to_string(),
        ))
    }
}
...
fn get_string(buf: &[u8]) -> io::Result<(String, &[u8])> {
    ...
    Err(new_err(
        "Bad format: String terminator not found".to_string(),
    ))
}
...
fn read_n<R: Read>(reader: &mut R, n: usize) -> io::Result<Vec<u8>> {
    ...
    if n == n_read {
        ...
        Err(new_err("Unexpected number of bytes read".to_string()))
    }
}
